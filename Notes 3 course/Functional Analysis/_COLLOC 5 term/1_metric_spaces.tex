...
\item Проверим полноту.
	...
	Обозначим $ f(x) \coloneq \lim\limits_{n \to \infty} f_n(x) $ (поточечный).
...
\begin{proof}
	При фиксированном $ p $ можно считать, что $ (F, \|\cdot\|_p) \sub l^p $
	(подпространство в алгебраическом смысле).

	Проверим, что оно не замкнуто.
	Возьмём
	$$ x = \set{\frac1{2^k}}_{k = 1}^\infty \in l^p \quad \forall 1 \le p \le +\infty $$
...
\begin{theorem}
	...
	$$ \forall \set{\mathtt D_n = \mathtt D_{r_n}(x)}_{n = 1}^\infty :
...
\begin{proof}
	...
	$$ L \xrightarrow \Phi l^\infty : \quad f \mapsto \set{f(a_j)}_{j = 1}^\infty $$

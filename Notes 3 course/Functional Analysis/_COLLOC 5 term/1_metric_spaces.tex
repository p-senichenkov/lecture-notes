...
\begin{definition}
	$ X $ "--- множество, $ \quad \rho : X \times X \to \R $

	$ (X, \rho) $ называется \emph{метрическим пространством}, если выполняются свойства:
	\begin{enumerate}
...
\item Проверим, что $ \|f\|_\infty $ удовлетворяет аксиомам нормы:
	...
	Тогда $ f(x), g(x) $ "--- числа.
	$$ \|f\|_\infty + \|g\|_\infty \ge |f(x)| + |g(x)| \trige |f(x) + g(x)| $$
...
\item Проверим полноту.
	...
	Из полноты $ \mathbb K $ следует, что существует поточечный предел $ \lim\limits_{n \to \infty}
	f_n(x) = f(x) $.
	...
	$$ \implies \lim\limits_{m \to \infty} f_n = f \text{ в пространстве } m(X) $$
\end{eproof}
...
\begin{definition}
	...
	\begin{enumerate}
		...
		\item \emph{Хаусдорфовость}: $ \forall a \ne b \in K \quad
			\exists U, V $ "--- открытые: $ \quad
			a \in U, ~ b \in V, \quad U \cap V = \O $.
	\end{enumerate}
\end{definition}
...
\begin{proof}
	...
	$$ \forall x_1 \ne x_2 \quad \forall 0 < \alpha < 1, ~
	\beta = 1 - \alpha \quad f\bigl(\alpha x_1 + (1 - \alpha)x_2 \bigr) > \alpha f(x_1) + (1 - \alpha)f(x_2) $$
...
\begin{statement}[неравенство Гёльдера]
	$ (T, \mathscr U, \mu), \quad f, g $ измеримы, $ \quad p > 1, \quad \frac1p + \frac1q = 1 $.

	\begin{equ}{ineq:1}
		\int\limits_T |fg| \di \mu \le \Bigl( \int\limits_T |f|^p \di \mu \Bigr)^{\frac1p} \cdot
		\Bigl( \int\limits_T |g|^q \di \mu \Bigr)^{\frac1q}
	\end{equ}
\end{statement}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item $ A = +\infty \implies \eref{ineq:1} $
		\item $ B = +\infty \implies \eref{ineq:1} $
		\item $ 0 < A, B < +\infty $
...
\item $ p > 1 $
	...
	\begin{itemize}
		...
		\item $ A < +\infty, ~ B < +\infty $
			...
			Возьмём $ a, b \in \R $. Понятно, что $ |a + b| \le 2\max\{|a|, |b|\} $.
			$$ |a + b|^p \le 2^p \max\{|a|^p, |b|^p\} \le 2^p \bigl( |a|^p + |b|^p \bigr) $$
...
\begin{definition}
	...
	Функции на $ T $ будут элементами $ \mathbb K^n $.
...
\begin{definition}
	...
	$$ \R^n = \set{x = (x_1, \dots, x_n), ~ x_j \in \mathbb K}, \quad \|x\|_\infty = \max\limits_{1 \le j \le n} |x_j| $$
...
\begin{definition}
	...
	$$ l^\infty = \set{x = \set{x_j \in \mathbb K}_{j = 1}^\infty |
	\sup\limits_{j \ge 1}|x_j| < +\infty} $$
	$$ l^\infty = m(\N) \implies l^\infty \text{ "--- банахово} $$
\end{definition}

\begin{statement}
	$ \set{\nder[m]x \in l_n^p}_{m = 1}^\infty, \quad
...
\item $ \impliedby $
	...
	\lim \Bigl( \sum_{j = 1}^n |x_j - \nder[m]x_j|^p \Bigr)^{\frac1p} = 0 $$
	Для бесконечных так же берём $ \max $.
\end{iproof}

\begin{definition}
	$$ l^p = \set{x = \set{x_j}_{j = 1}^\infty \in \mathbb K |
...
\begin{remark}
	\hfill
	\begin{enumerate}
		\item $ \set{\nder[m]x \in l^p}_{m = 1}^\infty, \quad
			1 \le p \le +\infty, \quad x \in l^p $

			$$ \lim\limits_{m \to \infty} \|\nder[m]x - x\|_p \implies
...
\begin{definition}
	...
	$$ F = \set{x = (x_1, x_2, \dots, x_{N(x)}, 0, \dots) | x_j \in \mathbb K} $$
	На $ F $ рассматриваем норму $ \|\cdot\|_p, \quad 1 \le p \le +\infty $.
\end{definition}
...
\begin{proof}
	$ (F, \|\cdot\|_p) $ "--- подпространство $ l^p $ в алгебраическом смысле при фиксированном
	$ p $.
	Проверим, что оно не замкнуто.

	$$ x \coloneq \set{\frac1{2^k}}_{k = 1}^\infty \in l^p \setminus F \quad \forall 1 \le p \le +\infty $$
	$$ \nder[m]x \coloneq \Bigl( \frac12, \frac1{2^2}, \dots, \frac1{2^m}, 0, \dots \Bigr) \in F $$
	\begin{itemize}
		\item $ 1 \le p < +\infty $
...
\item $ \lambda \in \mathbb K, \quad \|\lambda f\|_p = |\lambda| \cdot \|f\|_p $;
...
\begin{definition}
	...
	В пространстве $ \mathrm L^p $ будем рассматривать $ \ol f $ "--- классы эквивалентности $ f $.
	$$ \|\ol f\|_p = \|f\|_{\mathscr L^p} $$
\end{definition}

\begin{statement}
	$ \|\cdot\|_{\mathrm L^p} $ "--- норма.
\end{statement}
...
\begin{definition}
	...
	$ f $ "--- измерима
	$$ f \in \mathscr L^\infty \iff \exists c > 0 : \quad |f(x)| \le c \text{ \ale} $$
...
\begin{proof}
	Возьмём $ e_m = \set{x | {} |f(x)| > \|f\|_\infty + \frac1m} \implies \mu e_m = 0 $.
	$$ e \coloneq \set{x | {} |f(x)| > \|f\|_\infty} = \bigcup e_m \implies \mu e = 0 $$
\end{proof}
...
\begin{theorem}[Фату]
	$ (T, \mathscr U, \mu), \quad g_n(x) $ измеримы, $ \quad g_n(x) \ge 0, \quad g_n \to g(x) $ \ale на $ T $
	$$ \int\limits_T g_n(x) \di \mu \le C \implies \int\limits_T g(x)\di \mu \le C $$
\end{theorem}

\begin{theorem}
	$ (T, \mathscr U, \mu), \quad 1 \le p \le \infty $
...
\item $ p < +\infty $
	...
	Докажем, что $ \exists S(x) = \sum_{k = 1}^\infty f_k(x) $, \ie $ S(x) \in \mathrm L^p : \quad
	\lim\limits_{n \to \infty} \|S - S_n\|_p = 0 $.

	Рассмотрим для начала сумму модулей:
	$$ \sigma_n(x) = \sum_{k = 1}^n |f_k(x)|, \quad \sigma(x) = \sum_{k = 1}^\infty |f_k(x)| $$
	...
	\end{rcases} \implies \int\limits_T |\sigma(x)|^p \di \mu \le C \implies
	\sigma(x) < +\infty ~\ale $$
	Значит, для \ale $ x \quad \exists \sum_{k = 1}^\infty f_k(x) = S(x) $.

	Воспользуемся критерием Коши для $ \sum_{n = 1}^\infty \|f_n\| \le C $:
	$$ \forall \eps > 0 \quad \exists N : \quad
	\forall m > n > N \quad \sum_{k = n + 1}^m \|f_k\|_p < \eps $$
...
\begin{eg}
	$$ \mathcal C[a, b], \quad
	\|f\|_p = \Bigl( \int_a^b |f(x)|^p \di x \Bigr)^{\frac1p}, \quad 1 \le p < +\infty $$
\end{eg}
...
\begin{proof}
	...
	$$ f \in \mathcal C[a, b] \to \set{g \in \mathrm L^p [a, b] |
	g(x) = f(x) ~ \ale \text{ по } \lambda} $$
	...
	\textbf{Пусть} такая $ f $ существует.
...
\item $ A = \ol A \implies X \setminus A $ открыто.
	$$ x_0 \notin A \implies x_0 \in X \setminus A \implies
	\exists r > 0 : \quad \mathtt B_r(x_0) \in X \setminus A \implies
	\forall y \in A \quad \rho(x_0, y) \ge r \implies \rho(x_0, A) \ge r $$
\end{eproof}
...
\item $ X $ ограничено
	...
	Понятно, что $ f_t \in m(X) $.

	Определим отображение $ \phi : X \to m(X): \quad \phi(t) = f_t $.
	...
	\begin{rcases}
		...
		|\rho(t, t) - \rho(s, t)| = \rho(t, s)
	\end{rcases} \implies \|\phi(t) - \phi(s)\|_\infty = \rho(t, s) $$
...
\begin{theorem}
	$ (X, \rho) $ "--- полное \textbf{тогда и только тогда}, когда
	$$ \forall \set{D_n = \mathtt D_{r_n}(x)}_{n = 1}^\infty :
...
\item $ \impliedby $
	...
	$$ D_k \coloneq \mathtt D_{\eps_k} (x_{n_k}) $$

	Проверим, что $ D_{k + 1} \sub D_k $.
	Возьмём $ y \in D_{k + 1} $.
	$$ \rho(x_{n_{k + 1}}, y) \le \frac1{2^{k + 1}} $$
	\begin{multline*}
		\rho(y, x_{n_k}) \trile \rho(x_{n_k}, x_{n_{k + 1}}) + \rho(x_{n_{k + 1}}, y)
		< \frac1{2^{k + 1}} + \frac1{2^{k + 1}} = \frac1{2^k} \implies y \in D_k \implies \\
		\implies D_{k + 1} \sub \mathtt D_k \implies
		\exists a \in \bigcap_{k = 1}^\infty D_k \implies \lim x_{n_k} = a
	\end{multline*}
\end{iproof}
...
\begin{eg}[подготовительный]
	$ \set{F_n}_{n = 1}^\infty, \quad F_n \sub \R, \quad F_n = \ol F_n, \quad F_{n + 1} \sub F_n,
	\quad \bigcap_{n = 1}^\infty F_n = \O $
	...
	\begin{enumerate}
		...
		\item Шары в $ X $.
			...
			\begin{itemize}
				...
				\item $ x < n \implies x \notin \mathtt D_n $
			\end{itemize}
			$$ \mathtt D_n = [n, +\infty), \quad \bigcap_{n = 1}^\infty \mathtt D_n = \O $$
	\end{enumerate}
\end{eg}

\begin{remark}
	Если $ (X, \|\cdot\|) $ нормировано, то требование стремления радиусов к нулю избыточно:
	$$ \text{полнота } \iff \bigcap_{n = 1}^\infty \mathtt D_n \ne \O \text{ при }
	\mathtt D_{n + 1} \sub \mathtt D_n $$
\end{remark}
...
\begin{proof}
	...
	$ E = \set{(x_1, \dots, x_n, 0, \dots) \mid x_j \in \Q, ~ n \in \N} $ "--- счётное всюду
	плотное в $ l^p $.
...
\begin{theorem}
	$ (X, \rho) $ сепарабельно, $ \quad Y \sub X $.

	Тогда $ Y $ сепарабельно.
\end{theorem}

\begin{proof}
	$$ \rho(x_n, Y) \coloneq \inf\limits_{y \in Y} \rho(x_n, y) $$
...
\begin{proof}
	...
	Рассмотрим $ L = \set{f : X \to \R | f(x) \in m(X), \quad f(x) \equiv[x \ne a_j] 0} $.
	$$ f \in L \quad \|f\| = \sup\limits_{x \in X} |f(x)| = \sup\limits_{j \in \N} |f(a_j)| $$
	$$ \Phi : L \to l^\infty : \quad f \mapsto \set{f(a_j)}_{j = 1}^\infty $$
	$ \Phi $ "--- изометрия $ \underimp{L \sub m(X)} l^\infty $ не сепарабельно $ \implies L $
	не сепарабельно.
...
\begin{proof}
	Пусть $ M_j \sub X $ "--- нигде не плотные, $ \quad M = \bigcup_{j = 1}^\infty M_j $.
...
\begin{definition}
	$ X $ "--- линейное пространство, $ \quad \set{x_\alpha \in X}_{\alpha \in A} $.

	$$ \mathscr L\set{x_\alpha} = \set{x = \sum_{j = 1}^n c_jx_{\alpha_j}} $$
	$ \mathscr L\set{x_\alpha} $ будем называть \emph{линейной оболочкой} $ \set{x_\alpha} $.
\end{definition}

\begin{definition}
	$ (X, \|\cdot\|) $.

	Будем говорить, что $ \set{x_\alpha}_{\alpha \in A} $ "--- \emph{полная система элементов},
	если $ \ol{\mathscr L\set{x_\alpha}} = X $.
\end{definition}

\begin{exmpls}
\item $ \mathcal C[a, b], \quad \set{x^n}_{n = 0}^\infty $
	$$ \mathscr L\set{x^n}_{n \ge 0} = \mathcal P = \set{p(x) = \sum_{k = 0}^n a_kx^k}, \quad
	\ol{\mathcal P} = \mathcal C[a, b] $$
...
\begin{proof}
	...
	Возьмём $ H = \set{x = \sum_{j = 1}^n c_jx_j | c_j \in \Q} $  "--- счётно,
	$ E \sub \ol H, \quad \ol E = X $.
\end{proof}
...
\begin{theorem}[Лебега, о предельном переходе]
	$ \set{h_n(x) \ge 0} $ "--- измеримые, $ \quad h_n(x) \le \Phi(x) $
	$$ \int\limits_T \Phi(x) \di \mu < +\infty, \quad
	h_n(x) \underarr{n \to \infty} F(x) $$
	(все утверждения \ale).
	...
	\begin{enumerate}
		...
		\item $ \set{\chi_e}_{
				\begin{subarray}{c}
					e \in \mathcal U, \\
					\mu e < +\infty
				\end{subarray}} $ "--- полное семейство в $ \mathrm L^p, \quad 1 \le p < +\infty $.
	\end{enumerate}
\end{theorem}
...
\begin{proof}
	...
	Возьмём $ n \in \N $.
	$$ e_k \define \set{x \in T | \frac k n \le f(x) < \frac{k + 1}n, \quad
	k = 0, 1, \dots, n^2 - 1}, \quad e_{n^2} \define \set{x \in T | n \le f(x)} $$
	$$ \implies T = \bigcup_{k = 0}^{n^2} e_k $$

	$$ g_n(x) \coloneq \sum_{k = 1}^{n^2} \frac k n \chi_{e_k}(x) $$
	...
	\begin{enumerate}
		...
		\item $ 1 \le p < +\infty $
			...
			$$ \Bigl( \int\limits_T |f - g_n|^p \di \mu \Bigr)^{\frac1p} \to 0 \implies
			f \in \ol{\mathscr L\set{\chi_e}}_{
				\begin{subarray}{c}
					e \in \mathcal U \\
					\mu e < +\infty
				\end{subarray}} $$

			Для произвольной $ f $ можно написать $ f = f_+ - f_- $.
	\end{enumerate}
\end{proof}
...
\begin{definition}
	...
	\begin{enumerate}
		...
		\item $ A, B \in \mathcal R, \quad B \sub A \implies
			\exists \set{e_j \in \mathcal R}_{j = 1}^n : \quad
			e_i \cap e_j = \O, \quad A \setminus B = \bigcup_{j = 1}^n e_j $.
	\end{enumerate}
\end{definition}
...
\begin{definition}
	...
	\begin{enumerate}
		\item $ \mu(\O) = 0 $;
		\item $ \set{e_j \in \mathcal R}_{j = 1}^\infty, \quad e_j \cap e_i = \O, \quad
			e = \bigcup_{j = 1}^\infty \in \mathcal R \quad \implies \quad
...
\begin{theorem}
	$ (T, \mathcal U, \mu) $ "--- пространство с мерой, $ \quad \mu $ "--- стандартное
	распространение с $ \mathcal R, \quad 1 \le p < +\infty $.
...
\begin{proof}
	...
	Возьмём $ \eps > 0 $.
	По определению $ \inf $,
	$$ \exists \set{e_j \in \mathcal R}_{j = 1}^\infty : \quad e_j \cap e_i = \O, \quad
	\mu E \le \sum_{j = 1}^\infty \mu e_j < \mu E + \eps $$

	Обозначим $ A = \bigcup e_j $.

	Так как ряд сходится, можно отбросить его начало:
	...
	Уже доказано, что
	$$ \ol{\mathscr L\set{\chi_E}}_{
		\begin{subarray}{c}
			E \in \mathcal U \\
			\mu E < +\infty
		\end{subarray}}^{\|\cdot\|_p} = \mathrm L^p(T, \mu) $$
...
\begin{proof}
	...
	Возьмём $ e \in \mathcal R $.
	$$ \forall \eps > 0 \quad \exists e_0 \in \mathcal R_0 : \quad
	e \sub e_0, \quad \lambda (e_0 \setminus e) < \eps $$
	$$ \implies \|\chi_{e_0} - \chi_e\|_p = \Bigl( \int\limits_{e_0 \setminus e} \di \lambda \Bigr)^{\frac1p} < \eps^{\frac1p} $$
	$$ \implies \ol{\mathscr L\set{\chi_e}}_{e \in \mathcal R} \ni \set{\chi_e}_{e \in \mathcal R}
	\implies \ol{\mathscr L\set{\chi_e}}_{e \in \mathcal R_0}^{\|\cdot\|_p} =
	\mathrm L^p(\R^n, \lambda) \underimp{\mathrm L^p(E) \sub \mathrm L^p(\R^n)}
	\mathrm L^p(E) \text{ сепарабельно} $$
\end{proof}
...
\begin{theorem}
	...
	Тогда $ \mathcal C(T) \cap \mathrm L^p(T, \mu) $ плотно в $ \mathrm L^p(T, \mu) $.
\end{theorem}

\begin{proof}
	Пусть $ e \in \mathcal U, \quad \mu e < +\infty $.
	Приблизим $ \chi_e $ непрерывными (по $ \|\cdot\|_p $).
	$$ \mu \text{ регулярна } \implies \forall \eps > 0 \quad
	\exists \text{ замкн. } F, \text{ откр. } G : \quad
	F \sub e \sub G, \quad \mu(G \setminus E) < \eps $$
	$$ \phi(x) \coloneq \frac{\rho(x, T \setminus G)}{\rho(x, T \setminus G) + \rho(x, F)} $$

	Пусть $ \rho(x, F) = 0 \implies x \in F, ~ x \notin T \setminus G \implies
	\rho(x, T \setminus G) \ne 0 \implies \phi \in \mathcal C(T) $.

	$$ \phi(x) =
	\begin{cases}
		0, \quad x \in T \setminus G \\
		1, \quad x \in F \\
		0 \le x \le 1, \quad \text{иначе}
	\end{cases} $$
	$$ |\phi(x) - \chi_e(x)| =
	\begin{cases}
		0, \quad x \in T \setminus G \\
...
\begin{implication}
	$ K \sub \R^n $ "--- компакт, $ \quad 1 \le p < +\infty $

	$$ \ol{\mathcal C(K)}^{\|\cdot\|_p} =
	\mathrm L^p(K, \lambda) $$
\end{implication}
...
\begin{statement}[секвенциальная компактность]
	...
	$$ \iff \forall \set{x_n \in K}_{n = 1}^\infty \quad \exists \set{x_{n_j}} : \quad
	\exists \lim\limits_{j \to \infty} x_{n_j} \in K $$
\end{statement}
...
\begin{remark}
	Для $ K \sub \mathbb K^n $ верно и обратное.
	В общем случае "--- нет.
\end{remark}

\begin{eg}
	$$ l^2 = \set{x = \set{x_j}_{j = 1}^\infty |
	\Bigl( \sum_{j = 1}^\infty |x_j|^2 \Bigr)^{\frac12} = \|x\|_2 < +\infty} $$
...
\begin{definition}
	...
	$$ \iff \forall \set{x_n \in A}_{n = 1}^\infty \quad
	\exists \set{x_{n_j}}_{j = 1}^\infty : \quad
	\exists \lim\limits_{j \to \infty} x_{n_j} \in X $$
\end{definition}
...
\begin{definition}
	$ A \sub X $ \emph{вполне ограничено}, если $ \forall \eps > 0 $ существует
	конечная $ \eps $-сеть для $ A $.
\end{definition}
...
\begin{exmpls}
\item $ A \sub \mathbb K^n $

	Докажем, что если $ A $ ограничено, то $ A $ вполне ограничено.

	$ A $ ограничено $ \implies \exists M > 0 : \quad \forall x = (x_1, \dots, x_n) \in A \quad
	|x_j| \le M $.

	Пусть $ Q = \set{x \in \mathbb K^n | {} |x_j| \le M} $.
	$$ Q = \bigcup_{j = 1}^N Q_j, \quad \operatorname{diam} Q_j < \eps, \quad
	F = \set{\text{вершины } Q_j} $$
	$ \implies F $ "--- $ \eps $-сеть.
\item $ l^2 $
	...
	Если $ F $ "--- $ \frac12 $-сеть для $ D $, то $ \forall j \quad
	\exists f_j \in F \cap \mathtt B_{\frac12}(e_j) $

	$ f \ne f_i \implies \set{f_j}_{j = 1}^\infty \sub F \implies F $ бесконечно.
\item $ l^2 $
	$$ \Pi = \set{x = \set{x_j}_{j = 1}^\infty | \ |x_j| \le \frac1{2^j}}
	\text{ "--- \emph{гильбертов кирпич (в $ \R^3 $ так устроены кирпичи)}} $$
	...
	$$ \underimp{\R^n \sub l^2} \exists F \sub \R^n \text{ "--- конечная $ \eps $-сеть} $$
...
\begin{properties}
	$ (X, \rho) $
	\begin{enumerate}
		\item $ A \sub X $ вполне ограничено $ \implies \ol A $ вполне ограничено.
...
\begin{lemma}
	$ (X, \rho), \quad \eps > 0, \quad A \sub X, \quad \exists $ конечная $ \eps $-сеть для $ A $.

	$$ \implies A = \bigcup_{j = 1}^n C_j, \quad C_j \cap C_i = \O, \quad
...
\item $ \implies $
	\begin{itemize}
		\item Проверим полноту.
			...
			\As $ A $ "--- компакт,
			$$ \exists \set{x_{n_j}}_{j = 1}^\infty : \quad \exists \lim\limits_{j \to \infty}
			x_{n_j} = a \in A $$

			По свойству фундаментальных последовательностей,
			$ \lim x_n = a \implies (A, \rho) $ "--- полное.
		\item Проверим вполне ограниченность.

			Возьмём $ \eps > 0 $.
			$$ A \sub \bigcup_{x \in A} \mathtt B_\eps(x) $$
			$ A $ "--- компакт $ \implies \exists \set{x_j \in A}_{j = 1}^n : \quad
			A \sub \bigcup_{j = 1}^n \mathtt B_\eps(x_j) \implies \set{x_j}_{j = 1}^n $ "--- $ \eps $-сеть.
	\end{itemize}
\item $ \impliedby $

		Возьмём $ \set{x_n \in A}_{n = 1}^\infty $.
		Докажем, что $ \exists \set{x_{n_j}} : \quad
		\exists \lim\limits_{j \to \infty} x_{n_j} \in A $.
		...
		Пусть $ x_{n_1} \in A_1 $.
		$$ \exists n_2 > n_1 : \quad x_{n_2} \in A_2, \quad \dots, \quad
		\exists n_m > n_{m - 1} : \quad x_{n_m} \in A_m $$
...
\begin{implication}
	...
	$$ \implies A \text{ относительно компактно} $$
\end{implication}
...
\begin{theorem}
	$ K $ "--- метрический компакт

	$ \Phi \sub \mathcal C(K) $ относительно компактно \textbf{тогда и только тогда}, когда
	\begin{enumerate}
...
\begin{proof}
	...
	\begin{itemize}
		\item $ \implies $
			\begin{enumerate}
				\item $ \Phi $ вполне ограничено $ \implies \Phi $ ограничено.
				\item Равностепенная непрерывность

					$$ \forall \eps > 0 \quad \exists \text{ конечная } \eps \text{-сеть }
					\set{\phi_j \in \mathcal C(K)}_{j = 1}^n $$
					$$ \phi_j \in \mathcal C \implies
					\exists \delta_j : \quad \forall x, y \in K : \rho(x, y) < \delta_j \quad
		...
		\item $ \impliedby $
			...
			Для этого достаточно доказать, что существует относительно
			компактная $ \eps $-сеть в $ m(K) $.

			Воспользуемся условием равностепенной непрерывности:
			\begin{equ}{arzela-ascoli:1}
				\exists \delta : \quad \forall \rho(x, y) < \delta \quad \forall f \in \Phi \quad
				|f(x) - f(y)| < \eps
			\end{equ}

			По лемме о разбиении,
			\begin{equ}{arzela-ascoli:2}
				\exists C_j : \quad K = \bigcup_{j = 1}^n C_j, \quad
				C_j \cap C_i = \O, \quad C_j \ne \O, \quad \operatorname{diam} C_j < \delta
			\end{equ}

			$$ \Psi \define \set{g(x) = \sum_{j = 1}^n y_j \chi_{C_j}(x) | y_j \in \mathbb K} $$
			$$ \| g(x) \|_\infty =
			...
			Рассмотрим
			$$ F : l_n^\infty \to \Psi : \quad F(y) = \sum_{j = 1}^n y_j \chi_{C_j}(x) $$
			...
			Выберем компакт
			$$ Q = \set{y = (y_1, \dots, y_n) | {} |y_j| \le M} \implies
			F(Q) \text{ "--- компакт в } m(K) $$
			...
			\begin{multline*}
				...
				g(x) = f(x_m) \implies \\
				\implies |f(x) - g(x)| = |f(x) - f(x_m)| \underset{
					\begin{subarray}{c}
						\eref{arzela-ascoli:1} \\
						\eref{arzela-ascoli:2} \implies \rho(x, x_m) < \delta
					\end{subarray}
				}< \eps
			\end{multline*}
	\end{itemize}
...
\begin{theorem}
	...
	\begin{enumerate}
		\item $ \exists M > 0, ~ \alpha, \beta > 0 : \quad
			\forall x, y \in K : \rho(x, y) < \beta \quad \forall f \in \Phi \quad
			|f(x) - f(y)| \le M \bigl( \rho(x, y) \bigr)^\alpha $
		\item $ K = [a, b], \quad
			\exists f'(x), \quad
			\exists L > 0 : \quad \forall f \in \Phi \quad
			|f'(x)| \le L \quad \forall x \in (a, b) $
		\item $ K \sub G \sub \R^n, \quad
			K $ "--- компакт, $ \quad
			G $ "--- открытое
			$$ \exists L > 0 : \quad \forall f \in \Phi \quad
			\forall x \in G \quad \exists \Bigl| \pder f{x_j}(x) \Bigr| \le L $$
		\item $ K \sub G \sub \Co, \quad
			K $ "--- компакт, $ \quad
			G $ "--- открытое, $ \quad
			\forall f $ аналитична в $ G $
			$$ \exists L > 0 : \quad \forall f \in \Phi \quad |f(z)| \le L $$
	\end{enumerate}
...
\item Пусть $ y, z \in K : \quad [y, z] \sub G $
	Рассмотрим
	$$ \Gamma(t) : \quad
	0 \le t \le 1, \quad
	...
	$$ \forall x \in K \quad h(x) > 0 $$
	$$ \exists \min\limits_{x \in K} h(x) \fed r > 0 $$
...
\item $ F \coloneq \Co \setminus G $
	$$ \exists r > 0 : \quad \forall z \in K \quad \rho(z, F) \ge r $$

	Возьмём $ \delta = \frac r3 $.
	$$ \gamma = \set{\zeta | {} |z - \zeta| = 2\delta} \sub G $$

	Возьмём $ z, w \in K : \quad \rho(z, w) < \delta $.
	...
	\Bigl| \frac{z - w}{(\zeta - z)(\zeta - w)} \Bigr| \underset{
		\begin{subarray}{c}
			|\zeta - z| = 2\delta \\
			|\zeta - w| \ge \delta
		\end{subarray}}<
	\frac{|z - w|}{2 \delta \cdot \delta} $$
	Теперь
	$$ |f(z) - f(w)| \le \frac1{2\pi} \frac{L |z - w|}{2 \delta^2} \cdot 2\pi \cdot 2\delta =
	\frac L \delta |z - w| $$
	Применяем случай 1 с $ \alpha = \beta = 1, ~ M = \frac L \delta $.
\end{eproof}

...
\begin{definition}
	$ X $ "--- \emph{линейное пространство} над $ \mathbb K $, если
	\begin{enumerate}
		\item $ x, y \in X \implies \alpha x + \beta y \in X \quad
...
\begin{definition}
	$ X, Y $ линейны над $ \mathbb K, \quad A : X \to Y $.
...
\begin{remark}
	$ \mathscr Lin(X, Y) $ "--- линейное пространство над $ K $.
\end{remark}
...
\item $ K(s, t) \in \mathcal C \bigl( [a, b] \times [a, b]\bigr) $
	...
	$$ ( \mathscr K f)(s) = \int_a^b K(s, t) f(t) \di t $$
	$ K(s, t) $ называется \emph{ядром интегрального оператора}.
	$$ \mathscr K \in \mathscr Lin \Bigl( \mathcal C[a, b] \Bigr) $$
\item $ X = \nder[1]{\mathcal C}[a, b], \quad Y = \mathcal C[a, b] $
...
\begin{definition}
	$ X $ "--- линейное пространство над полем $ \mathbb K, \quad B \sub X $.
...
\begin{theorem}
	$ X, Y $ "--- линейные пространства, $ \quad A \in \mathscr Lin(X, Y) $.

	\begin{enumerate}
		\item $ L $ "--- подпространство $ X \implies A(L) $ "--- подпространство $ Y $;
		\item $ M $ "--- подпространство $ Y \implies
			\underset{\text{(прообраз)}}{A^{-1}(M)} $ "--- подпространство $ X $;
...
\begin{definition}
	$ A \in \mathscr Lin(X, Y), \quad X, Y $ линейны над $ \mathbb K $.
...
\begin{definition}
	...
	$ A $ \emph{ограничен}, если $ \forall $ ограниченного $ B \sub X \quad A(B) $ ограничено.
\end{definition}
...
\begin{theorem}
	...
	\begin{enumerate}
		...
		\item $ A $ ограничен;
		\item $ \exists r > 0 : \quad A \bigl( \mathtt B_r(0) \bigr) $ ограничено.
	\end{enumerate}
\end{theorem}
...
\item 1 $ \implies $ 2
	...
	$ A $ непрерывен в $ \On[] \iff \exists \delta > 0 : \quad
	\forall \|x\| < \delta \quad \|Ax\| < \eps $.
...
\item 3 $ \implies $ 4
	...
	A(B) \text{ ограничено} $$
\item 4 $ \implies $ 5
	$$ \mathtt B_r(0) \text{ ограничено } \implies
	A \bigl( \mathtt B_r(0) \bigr) \text{ ограничено} $$
\item 5 $ \implies 1 $
	$$ \exists R > 0 : \quad A \bigl( \mathtt B_r(0) \bigr) \sub \mathtt B_R(0) $$
	\ie $ \|x\| < r \implies \|Ax\| < R $.

	$$ \delta \coloneq \eps \cdot \frac r R $$
	Пусть $ \|x\| < \delta $
	$$ \|x\| < \eps \cdot \frac r R \implies
	\Bigl\| x \cdot \frac R\eps \Bigr\| < r \implies
	\Bigl\| A \Bigl( x \cdot \frac R\eps \Bigr) \Bigr\| < R \implies
	\|Ax\| \cdot \frac R\eps < R \iff \|Ax\| < \eps $$
\end{iproof}
...
\begin{proof}
	...
	d = \sup\limits_{x \ne 0} $. \\
	Очевидно, что $ a \ge b, \quad a \ge c, \quad d \ge c $.
	...
	\begin{enumerate}
		...
		\item Пусть $ x \in X \ne 0 $
			...
			d \le \|A\| $$
		\item Пусть $ z \in X \ne 0 $
			$$ \Bigl\| \frac z{\|z\|} \Bigr\| = 1 \implies
			\Bigl\| A \Bigl( \frac z{\|z\|} \Bigr) \Bigr\| \le c \implies
			\|Az\| \le c\|z\| \implies
			\|A\| \le c $$
	\end{enumerate}
\end{proof}
...
\item $ X = \mathcal C[a, b], \quad h(x) \in \mathcal C[a, b] $
	...
	Проверим, что $ M_h \in \mathscr B(\mathcal C[a, b]), \quad
	\|M_h\|_{\mathscr B(\mathcal C[a, b])} = \|h\|_\infty $.
	$$ x \in [a, b] \implies \bigl( M_h(f) \bigr)(x) = h(x)f(x) \implies
	...
	\|M_h\|_{\mathscr B(\mathcal C[a, b])} \le \|h\|_\infty $$

	Пусть $ f(x) = \chi_{[a, b]}(x) \in \mathcal C[a, b] $.
	$$ \|\chi_{[a, b]}\|_\infty = 1 $$
	$$ \|M_h\| = \sup\limits_{\|f\| = 1} \|M_h(f)\|_\infty \ge \|M_h(\chi_{[a, b]})\|_\infty =
	\|h\|_\infty \implies
	\|M_h\|_{\mathscr B(\mathcal C[a, b])} = \|h\|_\infty $$
\item $ Y = \mathcal C[0, 1], \quad X = \Bigl( \mathcal C^{(1)}[0, 1], \ \|f\|_X =
	\max\limits_{[0, 1]}|f(x)| \Bigr) $

	$$ f \in X, \quad \mathcal D(f) = f', \quad \mathcal D \in \mathcal Lin(X, Y) $$
	Проверим, что $ \mathcal D $ не ограничен.
	$$ x^n \in \mathcal C[0, 1], \quad \|x^n\|_\infty = 1 \implies
	\sup\limits_{\|f\| = 1} \|\mathcal D(f)\|_\infty \ge \sup\limits_{n \in \N} \|\mathcal D(x^n)\| $$
	$$ \mathcal D(x^n) = nx^{n - 1} \implies \|\mathcal D(x^n)\|_\infty = n $$
	$$ \implies \sup \|\mathcal D(f)\| \ge \sup \|\mathcal D(x^n)\| = +\infty $$
\item $ Y = \mathcal C[0, 1], \quad X = \nder[1]{\mathcal C}[0, 1] $
	$$ \|f\|_X = \max\{\|f\|_\infty, \|f'\|_\infty\} $$
	$$ \mathcal D(f) = f' $$
	...
	\begin{itemize}
		\item Возьмём $ f \in X $.
			...
			$$ \|\mathcal D(f)\| = \max\limits_{[0, 1]}|f(x)| = \|f\|_\infty \le
			\max\{\|f\|_\infty, \|f'\|_\infty\} $$
			$$ \implies \mathcal D \in \mathscr B(X, Y), \quad \|D\| \le 1 $$
		\item Пусть $ f(x) = x \in X = \nder[1]{\mathcal C[0, 1]} $
			$$ \|f\|_\infty = 1, \quad \|f'\| = 1 $$
			$$ \implies \|\mathcal D(f)\| = 1, \quad \|f\|_X =
			\max\{\|f\|_\infty, \|f'\|_\infty\} = 1 $$
...
\section{Теоремы вложения для \texorpdfstring{$ l^p $}{l\textasciicircum p} и для
...
\begin{proof}
	...
	$$ \Bigl( \sum_{n = 1}^\infty |x_n|^{p_1} \Bigr)^{\frac1{p_1}} = 1 \implies
	|x_n| \le 1 \quad \forall n $$

	\begin{itemize}
		\item $ p_2 < +\infty $
			$$ \frac{p_2}{p_1} > 1 \implies
...
\item $ p_2 = +\infty $
	...
	$$ \implies \|A\| \le \Bigl( \mu(T) \Bigr)^{\frac1{p_1}} $$
\item $ p_2 < +\infty $
	\begin{multline*}
...
\begin{theorem}\label{th:full:1}
	$ (X, \|\cdot\|), \quad Y $ "--- банахово.

	Тогда $ \mathscr B(X, Y) $ "--- банахово.
\end{theorem}

\begin{proof}
	Возьмём фундаментальную $ \set{A_n \in \mathscr B(X, Y)}_{n = 1}^\infty $.
...
\begin{definition}
	...
	$ X^* = \mathscr B(X, \mathbb K) $ называется \emph{сопряжённым пространством}.
...
\item $ l^p, \quad $ зафиксируем $ i \in \N $
	...
	$$ |f(x)| = |x_i| \le \Bigl( \sum_{n = 1}^\infty |x_n|^p \Bigr)^{\frac1p} = \|x\|_p, \quad
	1 \le p < +\infty $$
	$$ |x_i| < \sup\limits_{n \ge 1}|x_n| = \|x\|_\infty, \quad p = +\infty $$
	$$ \implies f \in (l^p)^*, \quad \|f\|_{(l^p)^*} \le 1 $$
...
\begin{theorem}
	$ \phi(x) \in \mathcal C [a, b] $ "--- фиксирована, $ \quad f \in \mathcal C[a, b] $.
...
\item $ f = \chi_{[a, b]} $
	...
	\begin{itemize}
		...
		\item $ \phi $ "--- произвольная
			...
			t_k - t_{k - 1} < \delta $$
			$$ s, t \in [t_{k - 1}, t_k] \implies |\phi(s) - \phi(t)| < \eps $$
			$$ \mathscr F \coloneq \set{[t_{k - 1}, t_k]}_{k = 1}^n $$
			Разобьём отрезки на два множества:
			\begin{enumerate}
				\item $ \Delta_1, \dots, \Delta_r $ "--- отрезки такие, что $ \phi(t) \overset{\Delta_j}> 0 $
			...
			\begin{multline*}
				...
				\le \eps \Bigl( \sum_{j = r + 1}^n |\Delta_j| \Bigr) \le
				\eps(b - a)
			\end{multline*}
			$$ h(t) =
			\begin{cases}
			...
			\end{cases} $$
			Если $ [a, t_1] $ "--- отрезок второго сорта, то $ h(a) \coloneq 0 $.
			Аналогично для $ [t_{n - 1}, b] $.
			...
			\end{multline*}
			$$ \implies \|G\| \ge \int_a^b |\phi(t)| \di t $$
	\end{itemize}
\end{iproof}
...
\item $ \|\mathscr K\| \le M $
	...
	\begin{multline*}
		...
		M \cdot \|f\|_\infty \implies \|\mathscr K\| \le M
	\end{multline*}
\item $ \|\mathscr K\| \ge M $
...
\begin{theorem}
	...
	$ A $ "--- линейный изоморфизм \emph{тогда и только тогда}, когда
	\begin{enumerate}
...
\item $ \implies $
	...
	C_1 = \frac1{\|A^{-1}\|} $$
\item $ \impliedby $
	$$ \|Ax\| \le C_2 \|x\| \implies \|A\| \le C_2 \implies A \in \mathscr B(X, Y) $$
...
\begin{definition}
	$ X $ "--- линейное пространство над $ \mathbb K, \quad
	\|\cdot\|_1, ~ \|\cdot\|_2 $\ "--- различные нормы на $ X $.

	$ \|\cdot\|_1 $ \emph{эквивалентна} $ \|\cdot\|_2 $, если
	$$ \forall \set{x_n \in X}_{n = 1}^\infty \quad \forall x_0 \in X \quad
...
\begin{theorem}
	$ (X, \|\cdot\|), ~ (Y, \|\cdot\|) $ "--- линейные над $ \mathbb K, \quad
	\dim X = \dim Y = n \in \N $.
...
\begin{proof}
	...
	$$ A \Bigl( \sum_{j = 1}^n c_je_j \Bigr) = \sum_{j = 1}^n c_jf_j \implies
	...
	\begin{itemize}
		...
		\item Найдём $ c > 0 : \quad \|Az\| \ge c\|z\| \quad \forall z \in l_n^2 $
			...
			$ S $ "--- компакт в $ l_n^2 $.
			$$ \exists \min\limits_{z \in S} g(z) \eqqcolon r > 0 \implies
...
\item $ A : X \to l_n^2 $ "--- линейный изоморфизм

	В $ l_n^2 $ компакты ограничены и замкнуты.
	$ A $ сохраняет компактность (как линейный изоморфизм), ограниченность (\as он ограничен) и
	замкнутость (\as он непрерывен).
\end{eproof}
...
\item $ U \in \mathscr Lin(X, Y) $
	...
	Положим $ T = UA $.
	$$ T \in \mathscr Lin(l_n^2, Y) = \mathscr B(l_n^2, Y) \underimp{U = TA^{-1}}
	U \in \mathscr B(X, Y) $$
\end{iproof}
...
\begin{definition}
	$ (X, \rho), \quad Y \sub X, \quad a \in X $
...
\begin{theorem}
	$ (X, \|\cdot\|), \quad L $ "--- конечномерное подпространство $ X $ в алгебраическом смысле
	\begin{enumerate}
		\item $ L $ замкнуто;
...
\item $ a \in X, \quad d \coloneq \rho(a, L) = \inf\|a - y\| $
...
\begin{lemma}
	$ (X, \|\cdot\|), \quad L \subsetneq X $ "--- замкнутое подпространство, $ \quad
	0 < \eps < 1 $
...
\begin{proof}
	...
	$$ \rho(z, L) = \inf\limits_{y \in L} \|z - y\| \eqqcolon d > 0 \quad
	\text{ (\as $ L = \ol L $)} $$
	$$ \exists y \in L : \quad d \le \|z - y\| < \frac d{1 - \eps} $$
	$$ x_0 \coloneq \frac{z - y}{\|z - y\|} $$

	Возьмём $ u \in L $.
	$$ \|x_0 - u\| = \Bigl\| \frac{z - y}{\|z - y\|} - u \Bigr\| =
	\frac{\bigl\|z - \overbrace{y - u \cdot \|z - y\|}^{\in L} \bigr\|}{\|z - y\|} \ge
...
\begin{implication}
	...
	$$ \exists \set{y_n \in L_n}_{n = 1}^\infty : \quad
	\begin{cases}
		\|y_n\| = 1, \\
...
\item $ \implies $
	...
	Положим $ L_n = \mathscr L \set{x_j}_{j = 1}^\infty $.
	...
	Это \textbf{противоречит} секвенциальной компактности $ \mathtt B $.
\end{iproof}
...
\item Норма
	...
	Ax_n = Ax \implies Vx = \lim Ax_n = Ax $.
...
\item Единственность
	...
	Vx = \lim Vx_n = \lim Ax_n = \lim Wx_n = Wx $$
\end{iproof}
...
\item
	\begin{itemize}
		\item $ \|\ol x\| = 0 \implies \rho(x, Y) = 0 \underimp{Y \text{ замкнуто}} x \in Y
			\implies \ol x = \ol 0 $
...
\item $ X $ "--- банахово
	...
	$$ \exists \set{z_n \in \ol x_n} : \quad \|z_n\| \le 2\|\ol x_n\| \implies
	\sum_{n = 1}^\infty \|z_n\| < +\infty $$
	По критерию полноты для $ X $ имеем, что $ \exists z = \sum z_n, \quad z \in X $.
...
\begin{definition}
	$ H $ "--- линейное пространство над $ \Co $.

	Отображение $ (\cdot, \cdot) : H \times H \to \Co $ называется \emph{скалярным произведением}, если
	\begin{enumerate}
...
\item
	$$ (x + y, x + y) + (x - y, x - y) =
	(x, x) + (x, y) + (y, x) + (y, y) + (x, x) - (y, x) - (x, y) + (y, y) = (x, x) + (y, y) $$
\item
	\begin{multline*}
		|(x, y) - (x_n, y_n)| \trile |(x, y) - (x_n, y)| + |(x_n, y) - (x_n, y_n)| =
...
\item $ H^2 = \set{f(z) = \sum_{n = 0}^\infty a_nz^n | \set{a_n}_{n = 0}^\infty \in l^2} $ "---
	...
	$ H^2 $ "--- гильбертово.
	$$ \set{a_n}_{n = 0}^\infty \in l^2 \implies \lim\limits_{n \to \infty} a_n = 0 \implies
...
\section{Существование и единственность элемента ближайшего приближения в подпространстве
гильбертова пространства.
Теорема о проекции на подпространство.
Следствия}
...
\item Существование
	...
	Применим лемму.
	$$ \|y_n - y_m\|^2 \le 2 \bigl( \underbrace{\|y_n - x\|^2}_{\to d^2}
	+ \underbrace{\|y_m - x\|^2}_{\to d^2} \bigr)
	- 4d^2 \underarr{n, m \to \infty} 0 $$
	\begin{multline*}
		\implies \Set{y_n} \text{ фундаментальна } \underimp{M \text{ замкнуто } \implies M
		\text{ полно}}
		\exists y \in M : \quad \lim\limits_{n \to \infty} y_n = y \implies \\
		\implies \underbrace{\lim\|x - y_n\|}_{d} = \|x - y\| \implies
		\|x - y\| = d
	\end{multline*}
\item Единственность
	...
	По лемме
	$$ \|y - z\|^2 \le 2 \bigl( \|y - x\|^2 + \|z - x\|^2 \bigr) - 4d^2 = 0 \implies y = z $$
\end{iproof}
...
\item Единственность
	...
	$$ u \coloneq y - y_1 \implies u = z_1 - z $$
...
\begin{definition}
	...
	\begin{enumerate}
		...
		\item $ \forall x \in X, ~ y \in Y \quad x \perp y $.
	\end{enumerate}
\end{definition}
...
\begin{definition}
	...
	$$ \underset M P h \coloneq x, \quad \underset{M^\perp}P h \coloneq y $$
\end{definition}
...
\item $ M \sub H, \quad x \in H $
	...
	\begin{enumerate}
		\item Проверим линейность и ограниченность
			...
			\sup\limits_{u \ne 0} \frac{\|Pu\|}{\|u\|} \ge \frac{\|Px\|}{\|x\|} = 1 \implies
			\|P\| = 1 $$
		\item $ P^2 = P $
...
\begin{implication}[неравенство Бесселя]

...
\begin{theorem}
	...
	\begin{enumerate}
		...
		\item $ M $ "--- подпространство $ Y \implies
			\underset{\text{(прообраз)}}{A^{-1}(Y)} $ "--- подпространство $ X $;
...
\item 3 $ \implies $ 4
	...
	A(B) \text{ ограничено} $$
\item 4 $ \implies $ 5
	$$ \mathtt B_r(\On[]) \text{ ограничено } \underimp{\text{очевидно}}
...
\item $ Y = \mathcal C[0, 1], \quad X = \set{f | f' \in \mathcal C[0, 1]} $
	...
	$$ \implies \sup \|\mathscr D(f)\| \ge \sup \|\mathscr D(x^n)\| = +\infty $$
...
\begin{theorem}
	$ (X, \|\cdot\|) $
	\begin{enumerate}
		\item $ L \sub X $ "--- конечномерное подпространство в алгебраическом смысле
			$ \implies L $ замкнуто;
...
\item $ \implies $
	...
	Значит, $ \not\exists \lim\limits_{j \to \infty} y_n $.
\end{iproof}

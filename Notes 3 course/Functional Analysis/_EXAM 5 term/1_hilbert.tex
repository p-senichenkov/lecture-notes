...
\section{Теорема о разложении элемента гильбертова пространства в ряд Фурье, условие
полноты ОНС}

\begin{theorem}
	$ H $ "--- гильбертово, $ \quad \set{e_n}_{n = 1}^\infty $ "--- ОНС

	Следующие условия равносильны:
	\begin{enumerate}
		\item $ x \in \ol{\mathscr L \set{e_n}}_{n = 1}^\infty $;
...
\section{Существование ОНБ и изоморфность сепарабельных гильбертовых пространств}
...
\begin{proof}
	...
	Пусть $ x_1 = \dots = x_{n_1 - 1} = 0, \quad x_{n_1} \ne 0 $.
	Положим $ z_1 = x_{n_1}, \quad L_1 = \mathscr L\set{z_1} $.
...
\begin{proof}
	...
	$$ Tx = \set{(x, f_n)}_{n = 1}^\infty $$
	В $ l^2 $ мы попадаем в силу равенства Парсеваля:
	$$ \|x\|^2 = \sum_{n = 1}^\infty |(x, f_n)|^2 \implies
	\begin{cases}
		Tx \in l^2, \\
	...
	$$ \implies \exists g = \lim g_n \implies
	g = \sum_{j = 1}^\infty y_jf_j \implies T(g) = \Set{y_j} = y $$
\end{proof}
...
\begin{exmpls}
\item $ \mathrm L_\R^2[-\pi, \pi], \quad \lambda $ "--- мера Лебега
	...
	\begin{cases}
		0, \quad n \ne m, \\
		\pi, \quad n = m,
	\end{cases} \quad \int_{-\pi}^\pi 1 \di x = 2\pi $$
	Значит, $ \Set{1, \cos nx, \sin nx} $ "--- ортогональная система.
	...
	в $ L^2 $.
	$$ \exists \delta > 0 : \quad
	\Bigl( \int_{\pi - \delta}^\pi |g(x)|^2 \di x \Bigr)^{\frac12} < \eps $$
	$$ h(x) \coloneq
	\begin{cases}
	...
	\eps + \|g - h\|_2 + \|h - T_n\|_2 < 2\eps + \eps \sqrt{2\pi} $$
	...
	Ряд Фурье сходится к функции в пространстве $ \mathrm L^2 $:
...
\item $ \mathrm L_\Co^2 [-\pi, \pi] $
	...
	\int_{-\pi}^\pi e^{\ii nx}e^{-\ii mx} \di x =
	\begin{cases}
		0, \quad n \ne m, \\
		2\pi, \quad n = m
	\end{cases} $$
	...
	$$ c_n = \frac1{2\pi} \int_{-\pi}^\pi f(x) e^{-\ii nx}\di x, \quad n \in \Z $$

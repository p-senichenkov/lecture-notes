...
\section{Теорема Рисса о представлении линейного функционала в гильбертовом пространстве}
...
\item $ y \in H $
	\begin{itemize}
		...
		\item Оценим $ \|f_y\| $
			$$ \|f_y x\| = |(x, y)| \underset{\text{К"--~Б}}\le \|y\| \cdot \|x\| \implies
...
\item $ f \in H^* $
	\begin{itemize}
		...
		\item $ f \ne \On[] $
			...
			$$ 1 = f_y(v) = (v, y) = (v, \alpha v) = \alpha \|v\|^2 \implies
...
\item Возьмём $ z \in X \setminus L $
	$$ L_1 \coloneq \mathscr L\set{L, z} = \Set{x + tz | x \in L, \quad t \in \R} $$
	...
	Тогда, в силу полноты $ \R $, между $ A $ и $ B $ будет $ c $.
...
\begin{eproof}
\item[2.] $ \mathcal P \coloneq \Set{(M, h)}_{L \sub M \sub X}: \quad
	...
	$$ \implies x, y \in M_\beta \underimp{M_\beta \text{ "--- подпространство}}
	ax + by \in M_\beta \subset
...
\begin{statement}\label{st:cont:1}
	...
	f \in \mathscr Lin(X, \R), \quad f(x) \le p(x) $
...
\begin{proof}[овеществление]
	...
	\begin{rcases}
		f(ax) = u(ax) + \ii v(ax) \\
		f(ax) = af(x) = a u(x) + \ii a v(x)
	\end{rcases} \implies
	\begin{cases}
		u(ax) = au(x) \\
		v(ax) = av(x)
	\end{cases} \implies u, v \in \mathscr Lin(L, \R) $$
	...
	$$ u(x) \le |f(x)| \le p(x) \quad \forall x \in L \underimp{\text{Х"--~Б}}
	\exists \phi \in \mathscr Lin(X, \R) : \quad
	\begin{cases}
		\phi \bigr|_L = u, \\
	...
	\undereq{\text{лин.}} \ii \bigl(\phi(x) + \ii \psi(x) \bigr) \implies
	g \in \mathscr Lin(X, \Co) $$

	Проверим подчинение.
	$$ x \in X, ~ g(x) \in \Co \quad g(x) = re^{\ii \theta}, \quad r \ge 0, ~ \theta \in \R $$
	$$ |g(x)| = r = g \bigl( xe^{-\ii \theta} \bigr) =
...
\begin{proof}
	...
	Применяем теорему Хана"--~Банаха или Боненблюста"--~Собчика:
	$$ \exists g \in \mathscr Lin(X, \mathbb K) : \quad
	\begin{cases}
		g \bigr|_L = f, \\
		|g(x)| \overset X \le p(x)
	\end{cases} $$
	$$ \implies |g(x)| \overset X \le M \cdot \|x\| \implies \|g\|_{X^*} \le M \implies g \in X^* $$
	$$ \|g\|_{X^*} = \sup\limits_{
		\begin{subarray}{c}
			x \in X \\
			\|x\| \le 1
...
\item
	...
	\item $ x_0 = 0 $
		...
		Возьмём произвольную $ g \in X^* : \quad \|g\| = 1 $.
		$$ g(x_0) = 0 $$
	\end{itemize}
\item Рассмотрим $ h \in X^* : \quad \|h\| \le 1 $.
...
\begin{implication}[расстояние до подпространства]
	...
	\item
		...
		\begin{cases}
			...
			g \bigr|_L \equiv 0;
		\end{cases} $$
	\item $ d = \max\left\{ |h(x_0)| \mid \|h\| \le 1, ~ h\bigr|_L \equiv 0\right\} $.
	\end{enumerate}
\end{implication}

\begin{eproof}
\item $ M \coloneq \mathscr Lin(x_0, L) = \Set{\alpha x_0 + y | y \in L} $

	Определим $ f : M \to \mathbb K : \quad f(\alpha x_0 + y) = \alpha $.
	$$ \implies f(y) = 0 \quad \forall y \in L, \quad f(x_0) = 1 $$
...
\item Возьмём $ h \in X^* : \quad \|h\| \le 1, ~ h \bigr|_L = 0 $.
...
\begin{proof}
	...
	x \in U_\alpha^{-1}(Y_n) \quad \forall \alpha \in A \implies x \in E_n \implies
...
\begin{implication}
	$ X $ "--- банахово, $ \quad (Y, \|\cdot\|), \quad
	\Set{U_n \in \mathscr B(X, Y)}_{n = 1}^\infty, \quad U = \slim U_n $
...
критерий сильной сходимости операторов}
...
\item $ \impliedby $
	...
	$$ \|U_nx - U_mx\| \le
	\underbrace{\|U_nx - U_nz\|}_{\le \|U_n\| \cdot \|x - z\| \le M\|x - z\| < M\eps} +
...
\begin{theorem}[Банах"--~Штейнгауз]
	...
	\begin{enumerate}
		\item $ \exists M > 0 : \quad \|U_n\| \le M \quad \forall n \in \N $;
		\item $ \exists \lim U_nx = Ux \quad \forall x \in E \sub X $, где $ E $ "--- полное в $ X $.
	\end{enumerate}
\end{theorem}
...
\item $ L \coloneq \mathscr L\Set{E}, \quad x \in L $
	$$ x = \sum_{k = 1}^m c_k x_k, \quad x_k \in E $$
...
\section{Обратный оператор к \texorpdfstring{$ I - A $}{I - A}.
...
\begin{proof}
	...
	$$ \exists S = \sum_{k = 0}^\infty A^k \in \mathscr B(X) $$
	$$ S_n \coloneq \sum_{k = 0}^n A^k $$
	$$ \|S_n\| \le \sum_{k = 0}^n \|A\|^k < \frac1{1 - \|A\|} \underimp{\|S - S_n\| \to 0}
	\|S\| \le \frac1{1 - \|A\|} $$
...
\item $ \implies $
	$$
	\begin{rcases}
		U(0) = 0 \implies 0 \in U \bigl( \mathtt B_1(0) \bigr) \\
...
\item $ \impliedby $
	...
	Сдвинем на $ U(x_0) $:
	$$ \underbrace{Ux_0 + \mathtt B_{rR}(0)}_{\mathtt B_{rR}(Ux_0) = \mathtt B_{rR}(y_0)} \sub
	Ux_0 + U \bigl( \mathtt B_R(0) \bigr) = U \bigl( x_0 + \mathtt B_R(0) \bigr) =
	U \bigl( \underbrace{\mathtt B_R(x_0)}_{\sub G} \bigr) $$
	То есть, $ y_0 $ содержится в $ U(G) $ с окрестностью.
\end{iproof}
...
\begin{proof}
	...
	\begin{itemize}
		\item При $ k = 1 $
			$$ \|y\| < \frac r 2 \implies y \in \mathtt B_{\frac r 2}(0) \sub
...
\begin{proof}
	...
	\exists a \in Y ~ \exists r > 0 : \quad \mathtt B_r(a) \sub \ol{U(B_1)} $$

	Чтобы воспользоваться леммой, нужно сдвинуть точку $ a $ в 0.
	Возьмём $ z \in X : \quad \|z\| < r $.
	$$
	\begin{rcases}
		a + z \in \mathtt B_r(a) \sub \ol{U(B_1)} \\
		a \in \ol{U(B_1)} \implies -a \in \ol{U(B_1)}
	\end{rcases} \implies z = (a + z) + (-a) \in
	\ol{U(B_1)} + \ol{U(B_1)} \sub \ol{U(B_1)} $$
	Воспользуемся подобием:
	$$ \mathtt B_{\frac r2}(0) \sub \ol{U(B_1)} \underimp{\text{лемма}}
	\mathtt B_{\frac r4}(0) \sub U(B_1) $$
\end{proof}
...
\begin{proof}
	...
	Значит,
	$$ \exists x \in X ~ \exists y \in Y : \quad
	\begin{cases}
		\lim x_n = x \text{ в } (X, \|\cdot\|_X) \\
		\lim Ux_n = y \text{ в } Y
	\end{cases} \underimp{U \text{ замкнут}} y = Ux \implies
	\lim\bigl(\|x_n - x\|_X + \|Ux_n - Ux\|_Y \bigr) = 0 $$
	$$ \implies \lim\|x_n - x\|_1 = 0 \implies (X, \|\cdot\|_1) \text{ банахово} $$
	\begin{multline*}
		\|x\|_X \le \|x\|_X + \|Ux\|_Y = \|x\|_1 \underimp{\text{т. об экв. нормах}}
		\exists A > 0 : \quad \|x\|_1 \le A\|x\|_X \implies \\
		\implies \|x\|_X + \|Ux\|_Y \le A\|x\|_X \implies \|Ux\| \le A\|x\| \implies
		U \in \mathscr B(X, Y)
	\end{multline*}
\end{proof}

\begin{eg}[$ U $ замкнут, но не непрерывен]
	$ X = \Bigl( \nder[1]{\mathcal C}[0, 1], \|\cdot\|_\infty \Bigr), \quad
	Y = \mathcal C[0, 1] $
	...
	f_n' \uniarr{[0, 1]} g $$
	$$ x^n \in X : \quad \|x^n\| = 1, \quad \mathcal D(x^n) = nx^{n - 1}, \quad
	\|\mathcal D(x^n)\| = n $$
	$$ \implies \sup\limits_{\|f\| \le 1} \|D(f)\| = +\infty \implies
	\mathcal D \not\in \mathscr B(X, Y) $$
\end{eg}

\section(Сопряжённые пространства к пространствам последовательностей и пространствам Лебега)
...
\item $ y \in l^q, \quad x \in l^p $
	...
	\|x\|_p \cdot \|y\|_q \implies \|F_y\| \le \|y\|, \quad F_y \in (l^p)^* $$
\item $ F \in \bigl( l^p \bigr)^*, \quad
	...
	\begin{cases}
		F(S_n) = \sum x_ky_k \\
		S_n \to x
	\end{cases} \underimp{\text{непр. } F}
	F(S_n) \to F(x) \implies F(x) = \sum_{k = 1}^\infty x_ky_k = F_y(x) $$
	...
	\begin{itemize}
		\item $ p > 1 \implies q < +\infty $
			...
			$$ x_k =
			\begin{cases}
				\ol y_k \cdot |y_k|^{q - 2}, \quad y_k \ne 0, \\
				0, \quad y_k = 0
			\end{cases} $$

			Рассмотрим последовательности
			$$ \nder x = \sum_{k = 1}^n x_ke_k = (x_1, \dots, x_n, 0, \dots) $$
			$$ F \bigl( \nder x \bigr) = \sum_{k = 1}^n x_ky_k = \sum |y_k|^2 \cdot |y_k|^{q - 2} =
			\sum |y_k|^q $$
			$$ \bigl\| \nder x \bigr\|_p^p = \sum_{k = 1}^n |x_k|^p =
			\sum \Bigl| \frac{\ol{y_k}}{|y_k|} \Bigr|^p |y_k|^{p(q - 1)} =
			...
			\Bigl( \sum |y_k|^q \Bigr)^{\frac1q} = \|y\|_q $$
		\item $ p = 1 \implies q = \infty $
...
\begin{remark}
	$ T : (l^q) \to \bigl( l^{1 \le p < \infty} \bigr)^* : \quad T(y) = F_y $

	Мы доказали, что $ T $ "--- линейный изометрический изоморфизм.
...
\begin{theorem}[сопряжённое к $ \mathrm L^p $]
	\hfill
	\begin{enumerate}
		\item $ 1 \le p \le +\infty, \quad g \in \mathrm L^q(X, \mu) $ "--- фиксирована, $ \quad
			\frac1p + \frac1q = 1, \quad h \in \mathrm L^p $
			$$ F_g(h) \coloneq \int\limits_X h(x) g(x) \di \mu $$

			$$ \implies F_g \in (\mathrm L^p)^*, \quad \|F_g\| = \|g\|_{\mathrm L^q} $$
		\item $ 1 \le p < +\infty, \quad F \in (\mathrm L^p)^* $

			$$ \exists! g \in \mathrm L^q : \quad F = F_g $$
	\end{enumerate}
\end{theorem}
...
\begin{proof}
	...
	\begin{itemize}
		\item $ 1 < p \le +\infty $
			...
			\int\limits_X |g|^q \di \mu = \|g\|_q^q $$
			$$ \|F_g\| = \sup\limits_{h \ne 0} \frac{\|F_g(h)\|}{\|h\|} \ge \frac{|F_g(U)|}{\|U\|} =
			\frac{\|g\|_q^q}{\|g\|_q^{\frac q p}} = \|g\|_q^{q - \frac q p} = \|g\|_q $$
		\item $ p = 1, ~ q = \infty $
			\begin{itemize}
				...
				\item $ \|g\|_\infty > 0 $
					...
					$$ A \coloneq \Set{x \in X | {} |g(x)| \ge c} \implies 0 < \mu(A) < \infty $$
					...
					Если бы все $ \mu e_j = 0 $, то $ \mu A = 0 $.
					Значит, $ \exists e = e_j : \quad 0 < \mu e < +\infty $
...
\begin{proof}
	...
	\begin{itemize}
		\item Проверим, что $ \pi(x) \in \mathscr Lin(X^*, \Co) $
			...
			$$ \bigl( \pi(x) \bigr)(\alpha f) \bydef (\alpha f)(x) = \alpha f(x) =
			\alpha \bigl(\pi(x) \bigr)(f) $$

			Для $ f, g \in X^* $
			$$ \bigl(\pi(x) \bigr)(f + g) \bydef (f + g)(x) = f(x) + g(x) =
		...
		\item Проверим, что $ \|\pi(x)\| = \|x\| $
			...
			$$ \|\pi(x)\| =
			\sup\limits_{\|f\| \le 1} \bigl| \bigl(\pi(x)\bigr)(f) \bigr| \ge
...
\begin{proof}
	$ X^* $ "--- банахово $ \implies X^{**} $ банахово $ \underimp{\pi \text{ "--- изометрическое
	вложение}} \ol{\pi(X)} $ "--- пополнение $ X $.
\end{proof}

\begin{definition}
	Пространство $ X $ называется \emph{рефлексивным}, если $ \pi(X) = X^{**} $, \ie $ \pi $
	сюръективно, \ie
	$$ \forall G \in (X^*)^* \quad \exists x \in X : \quad G = G_x = \pi(x) $$
\end{definition}

\begin{implication}
	$ X $ рефлексивно $ \implies X $ "--- банахово.
\end{implication}

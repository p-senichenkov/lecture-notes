...
\begin{theorem}[об эквивалентных нормах]
	$ (X, \|\cdot\|_1) $ "--- банахово, $ \quad (X, \|\cdot\|_2) $ "--- банахово, \\
...
\textcolor{gray}{Похоже, сопряжённого к $ \mathrm L^p $ не было...}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item Проверим, что $ \|\pi(x)\| = \|x\| $
			...
			\implies \pi(x) \in (X^*)^*, \quad \|\pi(x)\| \le \|x\| $$
...
\begin{proof}
	...
	$$ f \in X^* \implies \|f\| = \max\limits_{\|g\| \le 1}|g(f)| $$
	Но $ X $ рефлексивно $ \implies \Set{g \in X^{**} | \|g\| \le 1} =
	\Set{\pi(x) | x \in X, ~ \|x\| \le 1} $.
	$$ \|f\| = \max\limits_{\|x\| \le 1} \bigl|\bigl(\pi(x)\bigr)(f)\bigr| =
	\max\limits_{\|x\| \le 1} |f(x)| $$
\end{proof}
...
\begin{properties}
	$ (X, \|\cdot\|), ~ (Y, \|\cdot\|), \quad T \in \mathscr B(X, Y) $
	\begin{enumerate}
		\item $ T^* \in \mathscr (Y^*, X^*), \quad \|T^*\| = \|T\| $;
		\item $ \alpha \in \Co $
			$$ (\alpha T)^* = \alpha T^* $$
...
\textcolor{gray}{Следствий нет ни у меня, ни у Якова...}

\part{Теория Фредгольма}

\section{Условие разрешимости уравнения Фредгольма.
Замкнутость образа оператора Фредгольма}

Будем изучать гильбертовы пространства, так как там проще доказательства.
Всё это верно и для банаховых пространств.

\begin{definition}
	$ H $ "--- гильбертово, $ \quad T \in \mathrm{Com}(H), \quad S = I - T $

	$ S $ будем называть \emph{оператором Фредгольма}
\end{definition}

\begin{definition}
	Рассмотрим серию уравнений:
	\begin{enumerate}
		\item \emph{уравнение Фредгольма}: $ Sx = a $;
		\item \emph{однородное уравнение Фредгольма}: $ Sx = 0 $;
		\item \emph{сопряжённое уравнение Фредгольма}: $ S^*y = h $;
		\item \emph{однородное сопряжённое уравнение Фредгольма}: $ S^*y = 0 $.
	\end{enumerate}
\end{definition}

$$ \lambda \in \Co \ne 0, \quad V(\lambda) = \lambda I - T = \lambda \Bigl( I - \frac1\lambda T \Bigr) $$
$$ \frac1\lambda \in \mathrm{Com}(H) \implies I - \frac1\lambda T \text{ "--- оператор Фредгольма} $$
Значит, свойства $ S $ можно будет распространить на $ V(\lambda) $.

\begin{lemma}[об ограниченных проообразах]
	$ H $ "--- гильбертово, $ \quad T \in \mathrm{Com}(H), \quad S = I - T, $ \\
	$ \Set{y_n \in S(H)}_{n = 1}^\infty $ ограничена

	$$ \exists \Set{x_n}_{n = 1}^\infty : \quad \Set{x_n} \text{ ограничена}, \quad
	Sx_n = y_n $$
\end{lemma}

\begin{proof}
	$ M = \operatorname{Ker} S, \quad L = M^\perp, \quad H = M \oplus L $
	$$ S(M) = \Set{0} \implies S(H) = S(L) $$
	$$ y \in S(H) \implies \exists! x \in L : \quad Sx = y $$
	$$ \forall y_n \quad \exists x_n \in L : \quad Sx_n = y_n $$

	Проверим, что $ \Set{x_n} $ ограничена.
	\textbf{Пусть} это не так, \ie
	$$ \exists \Set{n_k}_{k = 1}^\infty : \quad \lim\|x_{n_k}\| = +\infty $$
	НУО будем считать, что сама последовательность $ \Set{x_n} $ обладает таким свойством:
	$$ \lim\|x_n\| = +\infty $$

	Возьмём последовательность
	$$ \left\{\frac{x_n}{\|x_n\|}\right\}_{n = 1}^\infty $$
	$$ T \in \mathrm{Com}(H) \implies \exists \Set{n_k} : \quad \exists
	\lim T \Bigl( \frac{x_{n_k}}{\|x_{n_k}\|} \Bigr) \eqqcolon z_0 $$
	\begin{multline*}
		y_{n_k} = S(x_{n_k}) = x_{n_k} - T(x_{n_k}) \implies
		\underbrace{\frac{y_{n_k}}{\|x_{n_k}\|}}_{\underarr{k \to \infty} 0} =
		\frac{x_{n_k}}{\|x_{n_k}\|} -
		\underbrace{T \Bigl( \frac{x_n}{\|x_{n_k}\|} \Bigr)}_{\underarr{k \to \infty} z_0}
		\implies \\
		\implies \lim\limits_{k \to \infty} \Bigl( \frac{x_{n_k}}{\|x_{n_k}\|} \Bigr) =
		z_0 \underimp{T \in \mathscr B(H)} \lim\limits_{k \to \infty} T
		\Bigl( \frac{x_{n_k}}{\|x_{n_k}\|} \Bigr) = T(z_0) \implies \\
		\implies z_0 = T(z_0) \implies S(z_0) = z_0 - Tz_0 = 0 \implies z_0 \in M =
		\operatorname{Ker} S
	\end{multline*}
	$$ \frac{x_{n_k}}{\|x_{n_k}\|} \in L \text{ "--- замкнуто}, \quad
	\lim \frac{x_{n_k}}{\|x_{n_k}\|} = z_0 \implies z_0 \in L, \quad \|z_0\| = 1 $$
	При этом, $ z_0 \in L \cap M \implies z_0 = 0 $ "--- \contra.
\end{proof}

\begin{theorem}
	$ H $ "--- гильбертово, $ \quad T \in \mathrm{Com}(H), \quad S = I - T $

	\begin{enumerate}
		\item $ S(H) $ замкнуто;
		\item $ S^*(H) $ замкнуто;
		\item $ H = S(H) \oplus \operatorname{Ker} S^* $;
		\item $ H = S^*(H) \oplus \operatorname{Ker}S $.
	\end{enumerate}
\end{theorem}

\begin{remark}
	3. и 4. эквивалентны тому, что

	Уравнение $ Sx = a $ разрешимо \textbf{для тех и только тех} $ a $, которые
	ортогональны решениям однородного сопряжённого уравнения Фредгольма $ S^*y = 0 $.
\end{remark}

\begin{eproof}
\item Возьмём $ \Set{y_n \in S(H)}_{n = 1}^\infty : \quad
	\exists \lim\limits_{n \to \infty} y_n = y_0 $.
	$$ \exists \lim y_n \implies \Set{y_n} \text{ ограничена } \underimp{\text{лемма}}
	\exists \Set{x_n \in H} : \quad \Set{x_n} \text{ ограничена}, \quad Sx_n = y_n $$
	$$ T \in \mathrm{Com}(H) \implies \exists \Set{n_k} : \quad \exists \lim Tx_{n_k} = z_0 $$
	$$ \underbrace{y_{n_k}}_{\to y_0} = S(x_{n_k}) = x_{n_k} - \underbrace{Tx_{n_k}}_{\to z_0} $$
	$$ \implies \exists \lim\limits_{k \to \infty} x_{n_k} = x_0
	\underimp{T \in \mathscr B(H)}
	\lim T(x_{n_k}) = Tx_0 $$
	$$ \implies z_0 = Tx_0 \implies y_0 = x_0 - Tx_0 = S(x_0) \implies y_0 \in S(H) $$
\item $ S^* = I - T^*, \quad T^* \in \mathrm{Com}(H) \implies S^*(H) $ замкнуто.
\item
	$$ \forall A \in \mathscr B(H) \quad H = \ol{A(H)} \oplus \operatorname{Ker} A^* $$
	$$ S(H) = \ol{S(H)} \implies H = S(H) \oplus \operatorname{Ker} S^* $$
\item Аналогично.
\end{eproof}

\section[Альтернатива Фредгольма: инъективность оператора Фредгольма эквивалентна его
\texorpdfstring{\\}{}
сюръективности]
{Альтернатива Фредгольма: инъективность оператора Фредгольма эквивалентна его
сюръективности}

\begin{lemma}[о стабилизации]
	$ H_0 = H, \quad \forall k \ge 0 \quad H_{k + 1} \coloneq S(H_k) $

	$$ \exists n \ge 0 : \quad H_n = H_{n + 1} $$
\end{lemma}

\begin{proof}
	\textbf{Допустим}, что $ H_{k + 1} \subsetneq H_k $.

	Только что доказали, что $ H_{n + 1} $ замкнуто в $ H_n $.
	При этом, $ S(H_n) $ "--- подпространство $ H_n $.
	$$ H_k = H_{k + 1} \oplus H_{k + 1}^\perp \text{ (ортогональное дополнение в $ H_k $)} $$
	$$ \exists x_k \in H_k : \quad \|x_k\| = 1, \quad x_k \perp H_{k + 1} $$

	Пусть $ n > m $.
	Хотим оценить разность $ \|Tx_n - Tx_m\| $.
	$$ Sx_n - Sx_m = x_n - Tx_n - x_m + Tx_m $$
	$$ \|Tx_n - Tx_m\| = \|\underbrace{x_n - Sx_n + Sx_m}_{\in H_{n + 1}} -
	\underbrace{x_m}_{\perp H_{n + 1}}\| \ge \|x_m\| = 1 $$
	Значит, не существует $ \Set{Tx_{n_k}} : \quad \exists \lim T_{x_{n_k}} $ "--- \contra с
	$ T \in \mathrm{Com}(H) $.
\end{proof}

\begin{theorem}
	$ \operatorname{Ker} S = \Set{0} \iff S(H) = H $
\end{theorem}

\begin{restate}
	Оператор $ S $ "--- инъекция \textbf{тогда и только тогда}, когда $ S $ "--- сюръекция.
\end{restate}

\begin{restate}
	\hfill
	\begin{itemize}
		\item \textbf{Либо} уравнение Фредгольма разрешимо для любой правой части и решение
			единственно.
		\item \textbf{Либо} существует ненулевое решение однородного уравнения Фредгольма.
	\end{itemize}
\end{restate}

\begin{iproof}
\item $ \implies $

	$ \operatorname{Ker} S = \Set{0} $, \ie $ S $ "--- инъекция.

	Докажем, что $ S(H) = H $.
	\textbf{Пусть} $ S(H) \subsetneq H $, \ie $ \exists x \in H : \quad x \notin S(H) $
	(\ie $ Sx \in H $).

	Подействуем на $ x $ оператором $ S $:
	$$ Sx \in S(H), \quad S(Sx) \notin S(H) $$
	(\as прообраз у элемента ровно один, и это "--- $ Sx $)
	$$ \implies Sx \in S(H) \setminus S^2(H) $$
	И так далее для любого $ n $:
	$$ S^nx \in S^n(H) \setminus S^{(n + 1)}(H) $$
	Это \textbf{противоречит} лемме.
\item $ \impliedby $
	$$
	\begin{rcases}
		\begin{rcases}
			S(H) = H \\
			H = S(H) \oplus \operatorname{Ker} S^*
		\end{rcases} \implies \operatorname{Ker} S^* = \Set{0} \\
		S^* = I - T^* \implies S^* \text{ "--- оператор Фредгольма}
	\end{rcases}
	\underimp{\text{только что доказано}} S^*(H) = H $$
	$$ H = S^*(H) \oplus \operatorname{Ker} S \implies \operatorname{Ker} S = \Set{0} $$
\end{iproof}

\section{Теорема о числе линейно-независимых решений однородного уравнения Фредгольма.
Следствие о спектре компактного оператора}

\begin{theorem}
	$ S = I - T, \quad T \in \mathrm{Com}(H) $

	$$ \dim \operatorname{Ker} S = \dim \operatorname{Ker} S^* < +\infty $$
\end{theorem}

\begin{restate}
	Однородные уравнения Фредгольма $ Sx = 0 $ и $ S^*y = 0 $ имеют одно и то же, и при том
	конечное, число ЛНЗ решений.
\end{restate}

\begin{proof}
	Пусть $ \dim \operatorname{Ker} S = n < +\infty $ (\as это собственное подпространство для
	компактного оператора)
	$$ \dim \operatorname{Ker} S^* \eqqcolon m < +\infty $$

	\textbf{Пусть} $ n < m $.

	Выберем в этих пространствах ОНБ:
	$$ \Set{e_i}_{i = 1}^n \text{ "--- ОНБ } \operatorname{Ker} S, \quad \Set{f_j}_{j = 1}^m
	\text{ "--- ОНБ } \operatorname{Ker} S^* $$

	Определим $ V : H \to \operatorname{Ker} S^* $:
	$$ x \in H \quad Vx = \sum_{k = 1}^n (x, e_k) f_k $$
	В $ \operatorname{Ker} S^* $ мы попадём, так как оно замкнуто относительно линейных комбинаций.
	$$ V(H) \sub \operatorname{Ker}S^* \implies \dim \bigl( V(H) \bigr) < +\infty \implies
	V \text{ "--- конечного ранга } \implies V \in \mathrm{Com}(H) $$
	$$ U \coloneq S + V = I - \underbrace{(T - V)}_{\in \mathrm{Com}(H)} \implies
	U \text{ "--- оператор Фредгольма} $$

	Для $ U $ верна альтернатива Фредгольма.
	Проверим, что $ \operatorname{Ker} U = \Set{0} $.
	Пусть $ x \in \operatorname{Ker} U $.
	$$ Ux = 0 \implies Sx + Vx = 0 $$
	$$
	\begin{rcases}
		Sx \in S(H) \\
		Vx \in \operatorname{Ker} S^*
	\end{rcases} \underimp{S(H) \perp \operatorname{Ker} S^*} Sx \perp Vx \implies \\
	\begin{cases}
		Sx = 0 \\
		Vx = 0
	\end{cases} $$
	$$ Vx = \sum_{k = 1}^n (x, e_k)  f_k = 0 \implies (x, e_K) = 0 \quad \forall k \implies
	x \perp \operatorname{Ker} S \underimp{x \in \operatorname{Ker} S} x = 0 $$

	По альтернативе Фредгольма $ U(H) = H $.
	Проверим, что это невозможно для $ n + 1 $, \ie что
	$$ f_{n + 1} \notin U(H) $$

	Возьмём $ x \in H $.
	$$ Ux = Sx + Vx $$
	$$
	\begin{rcases}
		f_{n + 1} \in \operatorname{Ker} S^* \implies f_{n + 1} \perp S(H) \implies
		f_{n + 1} \perp Sx \\
		Vx = \sum (x, e_k) f_k \underimp{f_{n + 1} \perp f_k} f_{n + 1} \perp Vx
	\end{rcases} \implies f_{n + 1} \in \bigl( U(H) \bigr)^\perp = \Set{0} \text{ "--- } \contra $$

	Тем самым, мы доказали, что
	$$ \dim \operatorname{Ker} S \ge \dim \operatorname{Ker} S^* \ge \dim \operatorname{Ker} S^{**}
	= \dim \operatorname{Ker} S $$
\end{proof}

\begin{implication}
	$ T \in \mathrm{Com}(H) $
	\begin{enumerate}
		\item $ \lambda \ne 0, \quad \lambda \in \sigma(T) $
			$$ \lambda \in \sigma_p(T) $$
		\item $ \lambda \in \sigma_p(T), \quad H_\lambda = \operatorname{Ker}(\lambda I - T), \quad
			Y_{\ol \lambda} = \operatorname{Ker}(\ol \lambda I - T^*) $

			$$ \dim H_\lambda = \dim Y_{\ol \lambda} < +\infty $$
		\item $ \sigma_p(T) \setminus \Set{0} = \Set{\lambda_n}_{n = 1}^N $

			При этом, если $ N = +\infty $, то $ \lim\limits_{n \to \infty} \lambda_n = 0 $.
	\end{enumerate}
\end{implication}

\begin{eproof}
\item $ \lambda \in \sigma(T) \implies V(\lambda) $ необратим.

	\textbf{Если} $ V(\lambda) $ "--- сюръекция, то
	$ \dim \operatorname{Ker} \bigl( V(\lambda) \bigr)* = \Set{0} $
	(\as $ H = V(\lambda)(H) \oplus \operatorname{Ker} \bigl( V(\lambda) \bigr)^* $).
	Но по теореме это означает, что $ \dim \operatorname{Ker} V(\lambda) = \Set{0} \implies
	V(\lambda) $ обратим "--- \contra.

	Значит, $ V(\lambda) $ не сюръекция $ \underimp{\text{альт. Фредгольма}}
	\operatorname{Ker} V(\lambda) \ne \Set{0} $.
\end{eproof}

\section{Простейшие свойства самосопряжённого оператора}

\begin{properties}
	$ H $ "--- гильбертово, $ \quad T \in \mathscr B(H), \quad T = T^* $ (\ie $ (Tx, y) = (x, Ty) $)

	\begin{enumerate}
		\item $ (Tx, x) \in \R \quad \forall x $;
		\item $ \sigma_p(T) \sub \R $;
		\item $ \lambda, \mu \in \sigma_p(T), \quad Tu = \lambda u, \quad Tv = \mu v $
			$$ u \perp v $$
		\item $ \|T\| = \sup\limits_{\|x\| = 1} |(Tx, x)| $;
	\end{enumerate}
\end{properties}

\begin{eproof}
\item
	$$
	\begin{rcases}
		(Tx, x) = \ol{(x, Tx)} \\
		(Tx, x) = (x, Tx)
	\end{rcases} \implies (x, Tx) \in \R $$
\item
	$$ \lambda \in \sigma_p(T) \implies \exists u \ne 0 : \quad u = \lambda u \implies
	(Tu, u) = (\lambda u, u) \implies
	\lambda = \frac{(Tu, u)}{\|u\|^2} \in \R $$
\item $ Tu = \lambda u, \quad Tv = \mu v, \quad \lambda \ne \mu, \quad u, v \ne 0 $
	$$
	\begin{rcases}
		(Tu, v) = (\lambda u, v) = \lambda (u, v) \\
		(Tu, v) = (u, Tv) = (u, \mu v) = \mu (u, v)
	\end{rcases} \underimp{\text{вычтем}} \underbrace{(\lambda - \mu)}_{\ne 0}(u, v) = 0 \implies
	u \perp v $$
\item $ Q = \sup\limits_{\|x\| = 1}|(Tx, x)| $
	\begin{itemize}
		\item Пусть $ x \in H, \quad \|x\| = 1 $.
			$$ |(Tx, x)| \underset{\text{К"--~Б}}\le \|Tx\| \cdot \|x\| \le
			\|T\| \cdot \underbrace{\|x\| \cdot \|x\|}_{= 1} = \|T\| \quad \forall x : ~ \|x\| = 1 $$
			$$ \implies Q = \sup\limits_{\|x\| = 1} |(Tx, x)| \le \|T\| $$
		\item Пусть $ u\in H, \quad u \ne 0 $.
			\begin{equ}{self_dual_elem_props}
				\Bigl\| \frac{u}{\|u\|} \Bigr\| = 1 \implies \Bigl| \Bigl\lgroup T
				\Bigl( \frac{u}{\|u\|} \Bigr), \frac{u}{\|u\|} \Bigr\rgroup \Bigr| \le Q \implies
				|(Tu, u)| \le Q\|u\|^2 \quad \forall u \in H
			\end{equ}

			Возьмём $ x, y \in H $.
			Применим \eref{self_dual_elem_props} к $ (x + y) $ и к $ (x - y) $:
			$$ \bigl( T(x + y), x + y \bigr) = (Tx, x) + (Tx, y) + (Ty, x) + (Ty, y) \le Q\|x + y\|^2 $$
			$$ -\bigl( T(x - y), x - y \bigr) = - \bigl( (Tx, x) - (Tx, y) - (Ty, x) + (Ty, y) \bigr) \le
			Q\|x - y\|^2 $$
			Сложим эти два неравенства:
			$$ 2 \bigl( (Tx, y) + (Ty, x) \bigr) \le Q \bigl( \|x + y\|^2 + \|x - y\|^2 \bigr) $$
			К правой части применим тождество параллелограмма, а левую преобразуем:
			\begin{multline*}
				(Tx, y) + (Ty, x) = (Tx, y) + (y, Tx) = (Tx, y) + \ol{(Tx, y)} = 2 \Re (Tx, y) \implies \\
				\implies 4 \Re (Tx, y) \le Q \cdot 2(\|x\|^2 + \|y\|^2)
			\end{multline*}

			Выберем $ x : \quad \|x\| = 1 $ такой, что $ Tx \ne 0 $.
			$$ y \coloneq \frac{Tx}{\|Tx\|} \implies \|y\| = 1 $$
			Подставим $ x, y $ в неравенство:
			$$ 4 \Re \Bigl( Tx, \frac{Tx}{\|Tx\|} \Bigr) \le 4Q \implies
			\|Tx\| \le Q \quad \forall x : ~ \|x\| = 1 \implies \|T\| \le Q $$
	\end{itemize}
\end{eproof}

\section{Существование собственного числа, модуль которого равен норме компактного
самосопряжённого оператора}

\begin{definition}
	$ T = T^* $
	$$ M \coloneq \sup\limits_{\|x\| = 1} (Tx, x), \quad m \coloneq \inf\limits_{\|x\| = 1}(Tx, x) $$

	$ M $ и $ m $ называются \emph{границами} оператора $ T $.
\end{definition}

\begin{remark}
	$$ \|T\| = \max\{|M|, |m|\} $$
\end{remark}

\begin{theorem}
	$ H $ "--- гильбертово, $ \quad T = T^*, \quad T \in \mathrm{Com}(H) $

	$$ \exists \lambda \in \sigma_p(T) : \quad |\lambda| = \|T\| $$
\end{theorem}

\begin{iproof}
\item $ \|T\| = |M| $
	$$ M = \sup\limits_{\|x\| = 1}(Tx, x) \implies \exists \Set{x_n}_{n = 1}^\infty : \quad
	\|x_n\| = 1, \quad \lim(Tx_n, x_n) = M $$
	$$
	\begin{rcases}
		m \le M \\
		\max\{|M|, |m|\} = |M|
	\end{rcases} \implies M > 0 $$
	$$ T \in \mathrm{Com}(H) \implies \exists \Set{n_k}_{k = 1}^\infty : \quad
	\exists \lim\limits_{k \to \infty} Tx_{n_k} = y $$

	Переименуем: $ \lim\limits_{n \to \infty} Tx_n = y $.
	Проверим, что $ y $ "--- с. в. $ T $ и $ Ty = My $.
	\begin{multline*}
		0 \le \|Tx_n - M \cdot x_n \|^2 = (Tx_n - Mx_n, Tx_n - Mx_n) = \\
		= \|Tx_n\|^2 - M \underbrace{(Tx_n, x_n)}_{\to M} - M \underbrace{(x_n, Tx_n)}_{\to M} +
		M^2 \underbrace{\|x_n\|^2}_{= 1} \underarr{n \to \infty} \|y\|^2 - M^2 \implies
		\|y\| \ge M
	\end{multline*}
	$$ \|y\| = \lim \|Tx_n\| \le \|T\| \cdot \|x_n\| = \|T\| = M \implies \|y\| \le M $$
	$$ \implies \lim M \cdot x_n = \lim Tx_n = y $$

	Воспользуемся тем, что $ T $ непрерывен:
	$$ M \cdot \lim x_n = y \implies M \underbrace{\lim Tx_n}_{= y} = Ty \implies
	Ty = My \implies M \in \sigma_p(T) $$

\item $ \|T\| = |m| $

	Рассмотрим оператор $ T_1 = -T $.
	$$ \sup\limits_{\|x\| = 1} (T_1x, x) = -m \implies \exists y \ne 0 : \quad T_1y = -my \implies
	Ty = my $$
\end{iproof}

\section{Теорема Гильберта\texorpdfstring{"--~}{--}Шмидта о представлении компактного
самосопряжённого оператора в виде суммы ортогональных проекторов}

\begin{theorem}
	$ H $ "--- гильбертово, $ \quad T = T^*, \quad T \in \mathrm{Com}(H),
	\quad \sigma (T) \setminus \Set{0} = \Set{\lambda_n}_{n = 1}^{0 \le N \le +\infty} $ \\
	$ \lambda_n \in \sigma_p(T), \quad H_{\lambda_n} = \operatorname{Ker}(\lambda_n I - T),
	\quad P_n $ "--- ортогональный проектор на $ H_{\lambda_n} $

	$$ \implies T = \sum_{k = 1}^N \lambda_k P_k $$

	Если $ N = +\infty $, то
	$$ \Bigl\|T - \sum_{k = 1}^n \lambda_k P_{\lambda_k}\Bigr\| \underarr{n \to \infty} 0 $$
\end{theorem}

\begin{proof}
	Возьмём $ \lambda \ne 0 $ из $ \sigma_p $.
	Рассмотрим $ \vawe T = T - \lambda P_\lambda $
	$$ L \coloneq H_\lambda^\perp $$
	$$ \vawe T\bigr|_{H_\lambda} = 0 \implies H_\lambda \sub \operatorname{Ker} \vawe T $$
	$$ \vawe T \bigr|_L = T \bigl|_L $$
	$$ T^* = T \implies (\vawe T)^* = \vawe T $$
	При этом, компактность сохраняется: $ \vawe T \in \mathrm{Com}(H) $.

	Рассмотрим $ \mu \ne \lambda $ из $ \sigma_p(T) $.
	$$ H_\mu \perp H_\lambda \implies H_\mu \sub L \implies \mu \in \sigma_p(\vawe T),
	\quad \operatorname{Ker}(\mu I - \vawe T) = \operatorname{Ker}(\mu I - T) $$
	$$ \operatorname{Ker} \vawe T = \operatorname{Ker} T \oplus H_\lambda $$

	По предыдущей теореме,
	$$ \exists \lambda_1 : \quad |\lambda_1| = \|T\| $$

	Уже доказано, что $ \sigma(T) \sub \Set{\lambda \in \Co | {} |\lambda| \le \|T\|} $.
	Значит, $ \lambda_1 $ "--- максимальное по модулю.
	\begin{itemize}
		\item Если $ \lambda_1 = 0 $, то $ T = \On[] $.
		\item Если $ \|T\| > 0 $, то рассмотрим $ T_1 = T - \lambda_1 \cdot P_1 $.
	\end{itemize}

	По одному из простейших свойств самосопряжённого оператора,
	$ |\lambda_1| = \sup\limits_{\|x\| = 1}|(Tx, x)| $.

	Можем вычислить $ |\lambda_2| $:
	$$ |\lambda_2| = \sup\limits_{\|x\| = 1}|(T_1x, x)| =
	\sup\limits_{\begin{subarray}{c}
		\|x\| = 1, \\
		x \perp H_{\lambda_1}
	\end{subarray}}|(Tx, x)| $$
	Кроме того, $ |\lambda_2| = \|T_1\| $.

	И так далее по индукции.
	\begin{itemize}
		\item Если $ \exists N : \quad T_N = 0 $, то
			$$ T = \sum_{k = 1}^{N - 1}\lambda_k P_k $$
		\item Иначе:
			$$ \Bigl\| T - \sum_{k = 1}^{N - 1} \lambda_kP_k \Bigr\| = |\lambda_N| $$
			Но $ \lim\limits_{N \to \infty}\lambda_N = 0 $, значит,
			$ T = \sum_{k = 1}^\infty \lambda_k P_k $
	\end{itemize}
\end{proof}

\section{Теорема Гильберта\texorpdfstring{"--~}{--}Шмидта о существовании ОНБ из собственных
векторов компактного самосопряжённого оператора}

\begin{theorem}
	$ H $ "--- гильбертово, сепарабельно, $ \quad T = T^*, \quad T \in \mathrm{Com}(H) $

	Тогда существует $ \Set{e_n}_{n = 1}^\infty $ "--- ОНБ из собственных векторов $ T $.
\end{theorem}

\begin{proof}
	$$ \sigma_p(T) \setminus \Set{0} = \Set{\lambda_n}_{n = 1}^N, \quad
	|\lambda_1| \ge |\lambda_2| \ge \dotsb $$

	Если $ N = \infty $, то $ \lim\limits_{n \to \infty} \lambda_n = 0 $.
	$$ H_n = \operatorname{Ker}(\lambda_n I - T) $$
	$$ \dim H_n = m_n, \quad 1 \le n < +\infty $$

	Пусть $ \Set{e_{n\,j}}_{j = 1}^N $ "--- ОНБ в $ H_n $.
	$$ L \coloneq \mathscr L\Set{e_{n\,j}}_{n = 1}^N, \quad 1 \le j \le m_n $$
	$$ M \coloneq L^\perp, \quad H = L \oplus M $$

	Проверим, что $ M = \operatorname{Ker} T $.
	$$ H_0 \coloneq \operatorname{Ker}(0 \cdot I - T) = \operatorname{Ker} T
	\perp H_{\lambda_n} \quad \forall n \ge 1 $$
	($ H_\lambda $ все друг другу ортогональны; ноль "--- частный случай)
	$$ x \in H_0 \implies x \in M, \quad \operatorname{Ker}T \sub M $$

	Возьмём $ z \in M $.
	$$ z \perp H_{\lambda_n} \quad \forall n \ge 1 \implies P_{H_\lambda} z = 0
	\underimp{\text{т. Г."--~Ш. о
	предст. опер.}} Tz = \sum_{n = 1}^\infty \lambda_n P_\lambda = 0 \implies
	z \in \operatorname{Ker} T $$

	Получили, что $ M = \operatorname{Ker} T = H_0 $.
	$$ H \text{ сепарабельно } \implies H_0 \text{ сепарабельно } \implies
	\exists \Set{e_{0~j}}_{1 \le j \le m_n} \text{ "--- ОНБ в } H_0 = M $$
	А в $ L $ ОНБ мы уже нашли.
	Значит, ОНБ $ H $ будет их объединением.
\end{proof}

\begin{remark}
	$ H $ "--- гильбертово пространство

	Тогда существует ОНБ в $ \ol{T(H)} $ из собственных векторов $ T $.
\end{remark}

\begin{proof}
	$$ H = \ol{T(H)} \oplus \operatorname{Ker} T^* \underimp{T = T^*}
	H = \ol{T(H)} \oplus \operatorname{Ker}T \implies L = \ol{T(H)} $$
	Для $ x \in \ol{T(H)} $
	$$ x = \sum_{n = 1}^N \sum_{j = 1}^{m_n} (x, e_{n_j})e_{n_j} $$
\end{proof}

\section{Компактность интегрального оператора с ядром из
\texorpdfstring{$ \mathrm L^2 $}{пространства Лебега}}

\begin{theorem}
	$ \mathrm L^2(X, \mu), ~ \mathrm L^2(Y, \nu) $ сепарабельны, $ \quad
	K(x, y) \in \mathrm L^2(X \times Y) $
	$$ (\mathscr K f)(x) = \int\limits_Y K(x, y)f(y) \di \nu $$
	$$ \implies \mathscr K \in \mathrm{Com} \bigl( \mathrm L^2 (Y), \mathrm L^2 (X) \bigr) $$
\end{theorem}

\begin{proof}
	Пусть $ \Set{\phi_n(x)}_{n = 1}^\infty $ "--- ОНБ в $ \mathrm L^2(X, \mu), \quad
	\Set{\psi_j(y)}_{j = 1}^\infty $ "--- ОНБ в $ \mathrm L^2(Y, \nu) $.

	Проверим, что $ \Set{\phi_n(x) \psi_j(y)}_{n, j \in \N} $ "--- ОНБ в
	$ \mathrm L^2(X \times Y) $.
	$$ \int\limits_X \phi_n(x) \ol{\phi_m(x)} \di \mu =
	\begin{cases}
		1, \quad n = m, \\
		0, \quad n \ne m,
	\end{cases} \quad
	\int\limits_Y \psi_i(y) \ol{\psi_k(y)} \di \nu =
	\begin{cases}
		1, \quad j = k, \\
		0 \quad j \ne k.
	\end{cases} $$
	\begin{itemize}
		\item Проверим ортогональность
			$$ \iint\limits_{X \times Y} \phi_n(x) \psi_i(y) \ol{\phi_m(x)} \ol{\psi_k(y)}
			\di \mu \di \nu =
			\int\limits_X \phi_n \ol\phi_m \di \mu \int\limits_Y \psi_j \ol\psi_k \di\nu =
			\begin{cases}
				1, \quad m = n, ~ j = k, \\
				0
			\end{cases} $$
		\item Проверим полноту

			Возьмём $ f \in \mathrm L^2(X \times Y) $.
			Пусть $ f \perp \Set{\phi_n(x) \psi_j(y)}_{n, j \in \N} $, \ie
			$$ \iint\limits_{X \times Y} f(x, y) \ol{\phi_n(x)} \ol{\psi_k(y)} \di\mu \di\nu = 0
			\quad \forall n, k $$

			Зафиксируем $ n $.
			\begin{multline*}
				\int\limits_Y \Bigl( \int\limits_X f(x, y) \ol{\phi_n(x)} \di \mu \Bigr)
				\ol{\psi_k(y)} \di \nu = 0 \implies
				\int\limits_X f(x, y) \ol\phi_n \di \mu \perp \psi_k(y) \quad \forall k \implies \\
				\implies \int\limits_X f(x, y) \ol \phi_n \di \mu = 0 \text{ \ale по } \nu \implies
				f(x, y) = 0 \text{ \ale по } \mu \implies
				f = \On \text{ в } \mathrm L^2(X \times Y)
			\end{multline*}
	\end{itemize}

	$$ K(x, y) = \sum_{1 \le i, j < +\infty} \alpha_{ij}\phi_i(x) \psi_j(y) $$
	Рассмотрим частичные суммы:
	$$ K_n(x, y) = \sum_{1 \le i,j \le n} \alpha_{ij} \phi_i(x) \psi_j(y) $$
	$$ \lim \|K(x, y) - K_n(x, y)\| = 0 $$

	Уже доказано, что
	$$ \|\mathscr K\|_{\mathscr B(\mathrm L^2(Y), \mathrm L^2(X))} \le
	\|K(x, y)\|_{\mathrm L^2(X \times Y)} $$

	Возьмём $ f \in \mathrm L^2(Y, \nu) $.
	$$ (\mathscr K_n f) = \int\limits_Y K_n(x, y) f(y) \di \nu =
	\sum_{1 \le i, j \le n} \alpha_{ij}\phi_i(x) \int\limits_Y f(y) \psi_j(y) \di \nu $$
	$$ L_n \coloneq \mathscr L\Set{\phi_j}_{j = 1}^n, \quad \dim L_n = n, \quad
	\mathscr K_n(f) \in L_n $$
	Значит, $ \mathscr K_n $ "--- оператор конечного ранга.
	$$ \implies \mathscr K_n \in \mathrm{Com} \bigl( \mathrm L^2(Y), \mathrm L^2(X) \bigr) $$
	$$ \|\mathscr K - \mathscr K_n\|_{\mathscr B(\mathrm L^2(Y), \mathrm L^2(X))} \le
	\|K(x, y) - K_n(x, y)\| \uniarr{\mathrm L^2(X \times Y)} 0 $$
	Операторы компактные, значит, их предел по норме тоже компактен.
\end{proof}

\begin{implication}
	$ \mathrm L^2(X, \mu) $ сепарабельно, $ \quad K(x, y) \in \mathrm L^2(X \times X), \quad
	K(y, x) = \ol{K(x, y)} $

	$$ \mathscr K \in \mathrm{Com} \bigl( \mathrm L^2(X) \bigr) $$
\end{implication}

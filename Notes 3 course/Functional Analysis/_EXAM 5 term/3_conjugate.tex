...
\begin{theorem}
	...
	\begin{enumerate}
		\item $ \mathscr K \in
			...
			\|\mathscr K\| \le M $;
		\item $ \mathscr K^* \in
			\mathscr B \bigl( \mathrm L^q(\mathbb R^n), \mathrm L^p(\mathbb R^m) \bigr) $
...
\item Воспользуемся теоремой Фубини:
	...
	\Bigl( \int\limits_{\R^m}|K(x, y)|^p \di y \Bigr)^{\frac1p} \cdot \|f\|_q $$
...
\begin{proof}
	...
	Возьмём $ x \in L^\perp $.
	$$ (x, T^*y) = 0 \iff (Tx, y) = 0 \iff Ty = 0 \iff y \in \operatorname{Ker} T $$
...
\item
	\begin{itemize}
		\item $ \mu \in \rho(T) $
			...
			$$
			\begin{rcases}
				\phi \bigl( V(\lambda) \bigr) = R(\lambda) \\
...
\item $ \mu \in \rho(T), \quad \lambda $ из некоторой окрестности $ \mu $
	...
	Возьмём $ F \in \bigl( \mathscr B(X) \bigr)^* $.
...
\item Непустота
	...
	$$ V(0) = -T \implies \exists T^{-1} \in \mathscr B(X)
	\underimp{\text{сл. о дост. кол-ве лин. функц.}}
	\exists F \in \bigl( \mathscr B(X) \bigr)^* : \quad F(T^{-1}) = \|T^{-1}\| \ne 0 $$
	Выберем $ g(\lambda) = F \bigl( R(\lambda) \bigr) $.

	$$
	\begin{cases}
		\exists R > 0 : \quad |g(\lambda)| \le 1 \text{ при } \lambda \ge R \\
		\exists M = \max\limits_{|\lambda| \le R} |g(\lambda)|
	\end{cases} $$
...
\begin{props}
	...
	\item $ T \in \mathrm{Com}(X, Y) $ \textbf{тогда и только тогда}, когда
		...
		\Set{n_k}_{k = 1}^\infty : \quad \exists \lim\limits_{k \to \infty}Tx_{n_k} \in Y $$
\end{props}
...
\begin{proof}
	...
	$$ T(B) \sub T(X) \implies T(B) \text{ ограничено } \underimp{T(X) \text{ конечномерно}}
	T(B) \text{ относительно компактно} $$
\end{proof}
...
\item $ T(X) $ "--- замкнутое подпространство $ Y $
	\begin{multline*}
		...
		\exists r > 0 : \quad \mathtt B_r^{T(x)}(0) \sub \underbrace{T(X)}_{\text{отн. комп.}}
...
\begin{eproof}
\item Очевидно.
\item \textbf{Пусть} $ 0 \in \rho(T) \iff V(0) = 0 \cdot I - T = -T \implies
	\exists T^{-1} \implies T(X) = X $ "--- \contra с первым пунктом.
\end{eproof}
...
\begin{proof}
	...
	\begin{enumerate}
		\item
			\begin{itemize}
				...
				\item Возьмём $ T, S \in \mathrm{Com}(X, Y) $.
					...
					$ T(B), T(S) $ относительно компактны и вполне ограничены.
					...
					\begin{multline*}
						E + F = \Set{e + f | e \in E, ~ f \in F} \text{ "--- конечная $ 2\eps $-сеть для }
						T(B) + S(B) \implies \\
						\implies T(B) + S(B) \text{ вполне ограничено и относительно компактно}
					\end{multline*}
		...
		\item $ X, Y, Z $
			\begin{enumerate}
				\item $ T \in \mathrm{Com}(X, Y) $
					$$ T(B) \text{ относительно компактно } \underimp{\text{непр. } S}
					S \bigl( T(B) \bigr) \text{ относительно компактно} $$
				\item $ T \in \mathscr B(X, Y) $
					$$ T(B) \text{ ограничено } \underimp{S \in \mathrm{Com}}
					S \bigl( T(B) \bigr) \text{ относительно компактно} $$
			\end{enumerate}
	\end{enumerate}
\end{proof}
...
\begin{proof}
	Достаточно доказать в одну сторону, так как $ T = T^{**} $.
	Пусть $ T \in \mathrm{Com}(H) $.
	...
	Возьмём ограниченную $ \Set{x_n \in H}_{n = 1}^\infty $:
...
\begin{theorem}
	$ X $ "--- банахово, $ \quad T \in \mathrm{Com}(X), \quad \lambda \in \sigma_p(T) $,
	$ \quad X_\lambda = \operatorname{Ker}(\lambda I - T), \quad \delta > 0 $
...
\begin{proof}
	...
	Пусть $ y_n \in L_n \setminus L_{n - 1} $.
	Тогда $ y_n = \alpha_n x_n + u_n, \quad \alpha_n \ne 0, ~ u_n \in L_{n - 1} $.
	$$ Ty_n = \alpha_n T x_n + T u_n = \alpha_n \lambda_n x_n + Tu_n =
	\lambda_n(\alpha_n x_n + u_n) -
	\underbrace{\lambda_n u_n + Tu_n}_{ \eqqcolon v_n \in L_{n - 1}} = \lambda_ny_n + v_n $$

	Пусть $ n > m $.
	$$ Ty_m \in L_{n - 1} \sub L_n $$
	$$ \|Ty_n - Ty_m\| = \|\lambda_ny_n + v_n - Ty_m\| =
	|\lambda_n| \Bigl\| y_n - \underbrace{\frac1{\lambda_n}(-v_n + Ty_m)}_{\in L_{n - 1}} \Bigr\|
	\ge \frac \delta 2 $$
	Таким образом, $ \Set{Ty_n} $ не фундаментальна.
	Значит, она не содержит фундаментальных подпоследовательностей "--- \contra с $ T \in
	\operatorname{Com}(X) $.
\end{proof}
...
\begin{implication}
	...
	\begin{enumerate}
		...
		\item $ N $ "--- количество собственных чисел.
			...
			$ \lambda_n $ можно занумеровать в порядке убывания модулей.
	\end{enumerate}
\end{implication}
...
\item Очевидно.
\item Очевидно.
\item $ E_n \coloneq \Set{\lambda \in \sigma_p(T) | {} |\lambda| \ge \frac1n}, \quad |E_n| <+\infty $

...
\item $ \xi \sim N(a, \sigma^2) $
    ...
    \begin{multline*}
        ...
        -tf_\zeta(t)
    \end{multline*}
    Приршли к дифференциальному уравнению
...
$$ \nder f(0) = \int_{-\infty}^{+\infty} x^n \ii^n \di F(x) = \ii^n \mathbb E \xi^n $$
...
\begin{theorem}[критерий Пойа]
    Если функция $ f(t) $ удовлетворяет следующим критериям, то она является характеристической:
    \begin{enumerate}
        \item $ f(0) = 1 $;
        \item $ f(t) $ непрерывна;
        \item $ f(t) $ "--- чётная;

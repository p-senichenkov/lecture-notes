...
\begin{statement}[неравенства Бонферрони]
	$$ \sum_{k = 1}^n P(A_k) - \sum_{1 \le k < l \le n} P(A_kA_l) \le
...
\textcolor{gray}{Пример Бернштейна "--- это про разноцветный тетраэдр.}
...
\begin{theorem}[локальная предельная]
	...
	Соотношение
    $$ \sqrt{npq}P_n(m) \underarr{n \to \infty} \phi \Bigl( \dfrac{m - np}{\sqrt{npq}} \Bigr), $$
	где
...
\begin{definition}
	...
	\frac{n!}{m_1! m_2! \cdots m_r!} p_1^{m_1} p_2^{m_2} \cdots p_r^{m_r}, \quad
	m_1 + m_2 + \dots + m_r = n $$
\end{definition}

...
\begin{definition}
    Для любого $ n $ и при любых значениях $ t_1, \dots, t_n \in T $ функции
    $$ F_{t_1, \dots, t_n}(x_1, \dots, x_n) = P\Set{\xi_{t_1} < x_1, \quad \dots, \quad
    \xi_{t_n} < x_n} $$
...
\begin{definition}
    ...
    $$ \forall n = 2, 3, \dotsc \quad \forall t_1 < \dots < t_n \quad
    \text{независимы приращения } \xi_{t_1}, \quad \xi_{t_2} - \xi_{t_1}, \quad \dots, \quad
    \xi_{t_n} - \xi_{t_{n - 1}} $$
\end{definition}
...
\begin{definition}
    ...
    $$ \xi_{t(1) + \tau}, \quad \dots, \quad \xi_{t(n) + \tau} $$
    не зависят от $ \tau $.
\end{definition}

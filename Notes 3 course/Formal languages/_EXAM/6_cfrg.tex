...
\TODO{Обобщённые регулярные выражения}
...
Рекуррентный алгоритм построения синтаксической граф-схемы (СГС).
Лемма 8.2}

Будем в дальнейшем алфавитом $ V $ считать объединение алфавитов нетерминалов, терминалов и пустой
буквы, то есть,
$$ V = N \cup T \cup \Set{\lambda} $$

\begin{itemize}
	\item Если $ \operatorname{m}(\alpha) \in N $, то $ \alpha $ "--- \emph{нетерминальная вершина}.
	\item Если $ \operatorname{m}(\alpha) \in T $, то $ \alpha $ "--- \emph{терминальная вершина}.
	\item Если $ \operatorname{m}(\alpha) = \lambda $, то $ \alpha $ "--- \emph{пустая вершина}.
\end{itemize}

Тут есть замудрёные словесные описания, но в целом всё понятно по картиночкам.
...
Определение множества начальных вершин \texorpdfstring{$ H(A) $}{H(A)} графа
\texorpdfstring{$ \Gamma_A $}{Г\textunderscore A} и множества конечных вершин
\texorpdfstring{$ K(A) $}{K(A)} графа \texorpdfstring{$ \Gamma_A $}{Г\textunderscore A}}

\TODO{Достижимые вершины}
...
\section{Понятие достижимости на множестве терминальных вершин в СГС.
...
Рассмотрим множестве всех вершин в синтаксической граф-схеме $ \Gamma_G $.
...
\begin{definition}
	...
	Состояние $ S \faktor S \xi $ называется \emph{переходом по символу} (\emph{переходным
...
\begin{definition}
	...
	\begin{itemize}
		...
		\item $ S = \bigcup S_I $, где $ S-i \in \Im_0 $.
	\end{itemize}
\end{definition}
...
Данное правило состоит четырёх частей ($ A_i $-фрагментов), разделённых знаком $ ; $.

\begin{algorithm}
	\hfill
	\begin{enumerate}
		\item Рассмотрим $ A_1 $-фрагмент $ A \mathnormal{,} r_{11} \mathnormal{,} A $.
			Используя данный фрагмент, мы можем вывести строки $ Ar_{11}A $, $ A r_{11}Ar_{11}A,
			\dotsc $
...
\section{Общая схема получения регулярного выражения, эквивалентного приведённой КС"=грамматике без
самовставлений.
Пример}

\TODO{Общая схема получения регулярного выражения, эквивалентного...}

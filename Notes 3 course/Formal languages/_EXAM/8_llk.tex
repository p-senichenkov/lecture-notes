...
\section{Определение и свойства \texorpdfstring{$ \operatorname{LL}(k) $}{LL(k)}-грамматики.
...
\textcolor{gray}{Этого тоже нет\dots}
...
\texorpdfstring{$ \operatorname{LL} $}{LL}. Теорема 2.3}]
{Достаточные признаки непринадлежности КС-грамматики \\
классу \texorpdfstring{$ \operatorname{LL} $}{LL}. Теорема 2.3}
...
\begin{theorem}
	Алгоритм производит правильный 1-предсказывающий алгоритм анализа для любой
...
\section[{Алгоритм 2.3: построение \texorpdfstring{$ k $}{k}-предсказывающего алгоритма анализа.
Пример 2.9.
Теорема 2.5}]
{Алгоритм 2.3: построение \texorpdfstring{$ k $}{k}-предсказывающего алгоритма \\
анализа.
Пример 2.9.
Теорема 2.5}
...
\begin{algorithm}[вычисление $ \sigma(A) $ для $ A \in V_N $]
	...
	\begin{enumerate}
		...
		\item Положим
			$$ \sigma_{i + 1}'(A, B) = \sigma_i'(A, B) \cup \Set{L \subset V_T^{*k} |
				\begin{array}{r}
					\exists A \to X_1 \dots X_m, \quad L' \in \sigma_i'(X_p, B), \quad
					X_p \in V_n, \\[0.5em]
					1 \le p \le m, \quad L = L' \oplus \operatorname{FIRST}_k^G(X_{p + 1} \dots X_m)
			\end{array}} $$
...
\begin{eg}
	...
	Сначала получаем
	$$ \operatorname{FIRST}_1^G(S) = \Set{\eps, a, b}, \quad \operatorname{FIRST}_1^G(A) =
	\Set{a, b} $$
	Затем строим последовательности $ \sigma_i'(X, Y) $.
...
\begin{algorithm}[вычисление $ \operatorname{FOLLOW}_k^G(A) $]
	...
	\begin{enumerate}
		...
		\item Пусть значения $ \phi_0(A, B), \dots, \phi_i(A, B) $ уже построены для всех
			$ (A, B) $.
			$$ \phi_{i + 1}(A, B) = \phi_i(A, B) \cup \Set{w \in V_T^{*k} |
				\begin{array}{l}
					\exists A \to X_1 \dots X_p X_{p + 1} \dots X_m, \quad X_p \in V_N, \\[0.5em]
					w \in \phi_i(X_p, B) \oplus_k \operatorname{FIRST}_k^G(X_{p + 1}, \dots, X_m)
			\end{array}} $$
		...
		\item Предполагая, что множество $ \operatorname{LL}(k) $-таблиц, необходимых для анализа в
			...
			\begin{enumerate}
				\item $ M(T_{A~L}, u) = x_0y_0' T_{A_1~L_1} \dots T_{A_m~L_m} x_my_m' $, если
					$$
					\begin{cases}
						T_{A~L}(u) = \bigl( A \to x_0 A_1 x_1 \dots A_m x_m, \Braket{Y_1, \dots,
						Y_m} \bigr), \\
						A \to x_0 A_1 x_1 \dots A_mx_m, y_0 A_1y_1 \dots A_m y_m \text{ "--- }
						i \text{-е правило схемы}
					\end{cases} $$
					Здесь $ y_i' = h(y_i) $.
				\item $ M(a, u) = \mathrm{pop} $, если $ a \in \Sigma $, $ u = av $,
					$ v \in \Sigma^{*k - 1} $.
				\item $ M(b', u) = \mathrm{pass} $ для всех $ u \in \Sigma^{*k} $.
					Такой управляющий элемент определяет переход

...
\begin{proof}
	...
	\begin{equ}{cauchy_domain:12}
		\bigg| \cint[z]{\curvedir{\gamma_{1j}}}{f(z) - f(z_{1j})} \bigg| \le \acint{\gamma_{1j}}{|f(z) - f(z_{1j})|} \underset{\eref{cauchy_domain:1}, \eref{cauchy_domain:11}}\le \acint[z]{\gamma_{1j}}\eps = \eps \cdot l ~ \gamma_{1j}
	\end{equ}

	Если $ z \in \sigma_{1j} $, то
	$$ |z - z_{1j}| \le |z_{1~j + 1} - z_{1j}| \underset{\eref{cauchy_domain:11'}}< \delta_1 $$
	\begin{equ}{cauchy_domain:13}
		\implies \bigg| \cint[z]{\curvedir{\sigma_{1j}}}{f(z) - f(z_{1j})} \bigg| \le \acint{\sigma_{1j}}{|f(z) - f(z_{1j})|} < \eps \cdot |z_{j + 1} - z_{1j}| \le \eps \cdot l ~ \gamma_{1j}
	\end{equ}
...
\begin{proof}
	...
	\begin{equ}{cauchy_formula:25}
		\cint[\zeta]{\curvedir{\sigma_\delta}}{\vphi(\zeta)} \bdefeq\phi \cint[\zeta]{\curvedir{\sigma_\delta}}{\frac{f(\zeta)}{\zeta - z}} = \cint[\zeta]{\curvedir{\sigma_\delta}}{\frac{f(\zeta)}{\zeta - z}} + \cint[\zeta]{\curvedir{\sigma_\delta}}{\frac{f(z)}{\zeta - z}}
	\end{equ}
	...
	\begin{equ}{cauchy_formula:29}
		\underimp{\eref{cauchy_formula:25}} \cint[\zeta]{\curvedir S}{ \frac{\vphi(\zeta)}{\zeta - z}} = \cint[\zeta]{\curvedir{\sigma_\delta}}{\frac{f(z)}{\zeta - z}} + 2\pi i f(z)
	\end{equ}
...
\begin{proof}
	\TODO{Доказательство теоремы о монодромии.}

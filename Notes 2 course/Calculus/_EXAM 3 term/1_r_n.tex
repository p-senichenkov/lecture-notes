...
\begin{eproof}
	...
	$$ \norm{AX}_n \le c \cdot \norm{X}_m \bydef[\le] c \quad \bdef[\iff]\sup \quad \sup\limits_{\norm{X} \le 1} \norm{AX}_n \le c $$
\end{eproof}
...
\begin{eproof}
	...
	\begin{equ}{oper_norm:7}
		AX = \column{a_{11}x_1 + \dots + a_{1m}x_m}{a_{n1}x_1 + \dots + a_{nm}x_m}
	\end{equ}
	\begin{remind}
		Неравенство КБШ:
...
\begin{proof}
	...
	\begin{itemize}
		...
		Всё это означает, что \bt{нужно доказать}, что
		...
		\begin{equ}{r_order:19}
			\mathcal{D}f(V)\clamp{V = Y + tH} = \bigg( f_{x_1}'(Y + tH), \dots, f_{x_n}'(Y + tH) \bigg)
		\end{equ}
		...
		По \bt{индукционному предположению},
...
\section{Формула Тейлора для функций нескольких переменных с ос\tpst{"-}{}татком в форме Пеано}
...
\begin{proof}
	...
	\end{equ}
	\begin{equ}{taylor_peano:7}
		H^\alpha \bdefeq{x^\alpha} h_1^{\alpha_1} \cdots h_n^{\alpha_n} \implies |H^\alpha| \le \norm{H}^{\alpha_1} \cdots \norm{H}^{\alpha_n} = \norm{H}^{|\alpha|} = \norm{H}^r \implies \frac{|H^\alpha|}{\norm{H}^r} \le 1
	\end{equ}
...
\begin{eg}[переход от дифференциала порядка 1 к дифференциалу порядка 2]
	\hfill \\
...
\begin{proof}
	Достаточно доказать \eref{diff_high_order:21}, \as для полож. определённой $ B $, форма $ -B(H) $ отрицательно определена \\
...
\begin{eproof}
	...
	\begin{equ}{extremum:212}
		A(tH_2^*) = t^2A(H_2^*) \bdefeq{p_2} -p_2t^2
	\end{equ}
...
\begin{theorem}
	...
	$ \bm{F \in \Cont[1]E}, \qquad Y_0 = F(X_0), \qquad \bm{\mc{D}F(X_0)} $ \bt{обратима}
...
\begin{proof}[определение множества $ U $]
	...
	\begin{equ}{reversible:25}
		\implies \exist r > 0 : \quad \forall X \in \B_r(X_0) \quad \norm{\mc{D}F(X) - A} < 2\lambda
	\end{equ}
...
\begin{figure}[!ht]
	\begin{tikzpicture}[>=Stealth]
		...
		\draw[->, blue, thick, name intersections={of=X_1 and circ}] (intersection-1) -- +(150:0.7) node[above]{$ (1 - t)X_2 $};
	\end{tikzpicture}
...
% so that `proof`s don't "stick" to each other:
...
\begin{proof}
	...
	\begin{equ}{reversible:12}
		l(X_*) \le l(X) \quad \forall X \in \ol{S}
	\end{equ}
...
\begin{iproof}[дифференцируемость $ \Phi $]
	\item Непрерывность
...
\begin{proof}[гладкость $ \Phi $]
	Пользуясь формулой \eref{reverse:26}, запишем матрицу Якоби $ \Phi $:
...
Говорят, что равенство \eref{implicit_lin:1} задаёт $ X $ как \it{неявную функцию от} $ Y $, в данном случае равную $ C(Y) $.
	...
	\begin{enumerate}
		...
		\begin{equ}{implicit:7}
			...
			\begin{bmatrix}
				S \\
				T
			\end{bmatrix} \right) = T \quad \underimp{\operatorname{def} \Psi} \quad \Psi
			\begin{barg}
				S \\
				T
			\end{barg} =
		...
		\begin{bmatrix}
			...
			Y_0
		\end{bmatrix} \right) $$
		Поэтому \eref{8} выполнено при $ T \in \B_\rho^m(Y_0) $ \\

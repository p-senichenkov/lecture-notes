...
\begin{longtable}{l | r c l}
	...
	Дистрибутивность & $ A \amp (B \vee C) $ & $ \iff $ & $ A \amp B \vee A \amp C $ \\
					 & $ A \vee B \amp C $ & $ \iff $ & $ (A \vee B) \amp (A \vee C) $ \\
...
Так построенная формула называется \emph{совершенной ДНФ}.
...
\begin{undefthm}{Идея доказательства.}
	...
	Отрицание формулы в ДНФ, после применения правил де Моргана, является формулой в КНФ.
\end{undefthm}
...
\begin{enumerate}
	...
	\item \emph{Правила вывода} "--- конечное множество отношений над формулами, для каждого из которых все аргументы, кроме последнего, называются \emph{посылками правила}, а последний аргумент "--- \emph{заключением правила}.
\end{enumerate}
...
\begin{definition}
	...
	$ \Gamma $ называется \emph{антецедентом} секвенции, $ \Delta $ "--- \emph{сукцедентом (консеквентом)} секвенции.
\end{definition}
...
$$ (\vdash \amp) ~ \frac{\Gamma \vdash \Delta_1~A~\Delta_2 \qquad \Gamma \vdash \Delta_1~B~\Delta_2}{\Gamma \vdash \Delta_1~(A \amp B)~\Delta_2} $$
$$ (\vdash \vee) ~ \frac{\Gamma \vdash \Delta_1~A~B~\Delta_2}{\Gamma \vdash \Delta_1~(A \vee B)~\Delta_2} $$
$$ (\vdash \to) ~ \frac{\Gamma_1~A~\Gamma_2 \vdash \Delta_1~B~\Delta_2}{\Gamma_1~\Gamma_2 \vdash \Delta_1~(A \to B)~\Delta_2} $$
$$ (\vdash \leftrightarrow) ~ \frac{\Gamma_1~A~\Gamma_2 \vdash \Delta_1~B~\Delta_2 \Gamma_1~B~\Gamma_2 \vdash \Delta_1~A~\Delta_2}{\Gamma_1~\Gamma_2 \vdash \Delta_1~(A \leftrightarrow B)~\Delta_2} $$
$$ (\vdash \oplus) ~ \frac{\Gamma_1~\Gamma_2 \vdash \Delta_1~A~B~\Delta_2 \Gamma_1~A~B~\Gamma_2 \vdash \Delta_1~\Delta_2}{\Gamma_1~\Gamma_2 \vdash \Delta_1~(A \oplus B)~\Delta_2} $$

\columnbreak
$$ (\neg \vdash) ~ \frac{\Gamma_1~\Gamma_2 \vdash \Delta_1~A~\Delta_2}{\Gamma_1~\neg A~\Gamma_2 \vdash \Delta_1~\Delta_2} $$
$$ (\amp \vdash) ~ \frac{\Gamma_1~A~B~\Gamma_2 \vdash \Delta}{\Gamma_1~(A \amp B)~\Gamma_2 \vdash \Delta} $$
$$ (\vee \vdash) ~ \frac{\Gamma_1~A~\Gamma_2 \vdash \Delta \qquad \Gamma_1~B~\Gamma_2 \vdash \Delta}{\Gamma_1~(A \vee B)~\Gamma_2 \vdash \Delta} $$
$$ (\to \vdash) ~ \frac{\Gamma_1~\Gamma_2 \vdash \Delta_1~A~\Delta_2 \qquad \Gamma_1~B~\Gamma_2 \vdash \Delta_1~\Delta_2}{\Gamma_1~(A \to B)~\Gamma_2 \vdash \Delta_1~\Delta_2} $$
$$ (\leftrightarrow \vdash) ~ \frac{\Gamma_1~\Gamma_2 \vdash \Delta_1~A~B~\Delta_2 \qquad \Gamma_1~A~B~\Gamma_2 \vdash \Delta_1~\Delta_2}{\Gamma_1~(A \leftrightarrow B)~\Gamma_2 \vdash \Delta_1~\Delta_2} $$
$$ (\oplus \vdash) ~ \frac{\Gamma_1~A~\Gamma_2 \vdash \Delta_1~B~\Delta_2 \qquad \Gamma_1~B~\Gamma_2 \vdash \Delta_1~A~\Delta_2}{\Gamma_1~(A \oplus B)~\Gamma_2 \vdash \Delta_1~\Delta_2} $$
\end{multicols}
...
\begin{proof}
	...
	$$ P_1 \dots P_k \vdash R_1 \dots R_m ~ A ~ T_1 \dots T_n, \qquad P_1 \dots P_k \vdash R_1 \dots R_m ~ B ~ T-1 \dots T_n $$
	равносильны соответственно
	$$ \neg P_1 \vee \dots \vee \neg P_k \vee R_1 \vee \dots \vee R_m \vee A \vee T_1 \vee \dots \vee T_n $$
	$$ \neg P_1 \vee \dots \vee \neg P_k \vee R_1 \vee \dots \vee R_m \vee B \vee T_1 \vee \dots \vee T_n $$
	Воспользуемся коммутативностью дизъюнкции:
	$$ \neg P_1 \vee \dots \vee \neg P_k \vee R_1 \vee \dots \vee R_m \vee T_1 \vee \dots \vee T_n \vee A $$
